...
- `summary.profiles.*.controlled_significance`: random baseline(현재 1/3) 대비 exact binomial tail p-value.
...
반복 seed의 95% confidence interval과 p-value까지 고려한 summary label은 `robust_*` prefix로 표시한다. CI가 넓거나 random baseline 대비 p-value가 약하면 `inconclusive`로 남겨 후속 실험을 요구한다.
