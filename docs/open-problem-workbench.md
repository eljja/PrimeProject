...
Each problem also includes a `primeproject.ai-proof-forge.v1` object. The forge is the non-reproduction layer: it asks the AI to invent or refine a mathematical object, express the object in a search grammar, run countermodel batteries, and target a formal theorem stub. It also includes a discovery loop that mutates candidate objects, records theorem pressure, runs verifiers, demotes blocked candidates, and keeps only open candidates that still need an infinite-bridge artifact. The loop now has an attack runbook, falsification scorecard, cross-problem synthesis map, ranked portfolio decision, and next theorem draft mirrored into the Lean-oriented `InfiniteBridge.lean` skeletons. Every candidate must state required outputs, failure exits, pass signals, fail signals, transfer tests, priority scores, stop rules, and proof objects before it can be promoted. It explicitly rejects mere replay of known finite checks. 한국어로 말하면, AI proof forge는 이미 알려진 계산을 다시 보여주는 장치가 아니라, 새 수학적 객체를 만들고 그 객체가 실제 장벽을 뚫는지 반례 생성기로 먼저 부숴보는 장치다. discovery loop는 후보 객체를 변형하고, 왜 막혔는지 남기고, 아직 살아 있는 후보가 어떤 새 정리를 요구하는지 추적한다.
