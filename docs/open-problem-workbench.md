...
Each problem also includes a `primeproject.ai-proof-forge.v1` object. The forge is the non-reproduction layer: it asks the AI to invent or refine a mathematical object, express the object in a search grammar, run countermodel batteries, and target a formal theorem stub. It also decomposes the next theorem into named sublemmas with roles, risks, proof artifacts, and failure tests, then marks the highest-risk lemma instead of hiding the decisive gap inside prose. The discovery loop mutates candidate objects, records theorem pressure, runs verifiers, demotes blocked candidates, and keeps only open candidates that still need an infinite-bridge artifact. The loop now has an attack runbook, falsification scorecard, cross-problem synthesis map, ranked portfolio decision, theorem decomposition, and next theorem draft mirrored into the Lean-oriented `InfiniteBridge.lean` skeletons. Every candidate must state required outputs, failure exits, pass signals, fail signals, transfer tests, priority scores, stop rules, proof objects, and decomposition closure rules before it can be promoted. It explicitly rejects mere replay of known finite checks. 한국어로 말하면, AI proof forge는 이미 알려진 계산을 다시 보여주는 장치가 아니라, 새 수학적 객체를 만들고 그 객체가 실제 장벽을 뚫는지 반례 생성기로 먼저 부숴보는 장치다. 또한 다음 정리를 보조정리 단위로 쪼개서, 어떤 보조정리가 가장 위험하고 어떤 실패 테스트를 통과해야 하는지 공개한다. discovery loop는 후보 객체를 변형하고, 왜 막혔는지 남기고, 아직 살아 있는 후보가 어떤 새 정리를 요구하는지 추적한다.
