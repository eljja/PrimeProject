...
Each problem also includes a `primeproject.ai-proof-forge.v1` object. The forge is the non-reproduction layer: it asks the AI to invent or refine a mathematical object, express the object in a search grammar, run countermodel batteries, and target a formal theorem stub. It explicitly rejects mere replay of known finite checks. 한국어로 말하면, AI proof forge는 이미 알려진 계산을 다시 보여주는 장치가 아니라, 새 수학적 객체를 만들고 그 객체가 실제 장벽을 뚫는지 반례 생성기로 먼저 부숴보는 장치다.
