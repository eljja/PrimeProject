...
Each problem also includes a `primeproject.ai-proof-forge.v1` object. The forge is the non-reproduction layer: it asks the AI to invent or refine a mathematical object, express the object in a search grammar, run countermodel batteries, and target a formal theorem stub. It also includes a discovery loop that mutates candidate objects, records theorem pressure, runs verifiers, demotes blocked candidates, and keeps only open candidates that still need an infinite-bridge artifact. The loop now has an attack runbook and falsification scorecard, so every candidate must state required outputs, failure exits, pass signals, and fail signals before it can be promoted. It explicitly rejects mere replay of known finite checks. 한국어로 말하면, AI proof forge는 이미 알려진 계산을 다시 보여주는 장치가 아니라, 새 수학적 객체를 만들고 그 객체가 실제 장벽을 뚫는지 반례 생성기로 먼저 부숴보는 장치다. discovery loop는 후보 객체를 변형하고, 왜 막혔는지 남기고, 아직 살아 있는 후보가 어떤 새 정리를 요구하는지 추적한다.
