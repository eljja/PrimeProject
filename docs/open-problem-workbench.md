...
English: This document specifies a proof workbench for four still-open problems: the Riemann Hypothesis, Collatz conjecture, Goldbach conjecture, and Twin Prime conjecture. PrimeProject does not claim proofs. It records bounded evidence, proof obligations, invalid shortcuts, AI-assisted search frontiers, AI breakthrough programs, AI proof forges, formal replay contracts, and the exact conditions required before any claim can move beyond `open_not_proven`.

한국어: 이 문서는 아직 미해결인 네 문제, 즉 리만가설, 콜라츠 추측, 골드바흐 추측, Twin Prime 추측을 위한 proof workbench 규격이다. PrimeProject는 증명을 주장하지 않는다. 대신 bounded evidence(유한 증거), proof obligation(증명 의무), invalid shortcut(잘못된 지름길), AI-assisted search frontier(AI 보조 탐색 전선), AI breakthrough program(AI 돌파 프로그램), AI proof forge(AI 증명 생성/반증 작업대), formal replay contract(형식 재생 계약), 그리고 `open_not_proven`을 넘기 위해 필요한 조건을 기록한다.
...
TICKET-264 proves three partial theorems and one exact route no-go while resolving none of the parent conjectures:

- the RH packet-sequence lag bound sharpens to `liminf S_n >= L-A_+-A_-`, with the two one-sided coefficients jointly optimal;
- pointwise cancellation of every fixed nonzero Weyl harmonic is equivalent to divergence of the explicit finite-data threshold cutoff `K_N`;
- for every fixed power of two, the special q=3 tie residue and total admit abstract non-tie count models, eliminating the whole fixed-two-adic-sufficiency route;
- all 38 unique-root convergents at or below the explicit Twin ninth-order threshold are exactly evaluated and unit-free, and the next denominator crosses the threshold.

The next single lemmas are `ActualWeilPacketOneSidedReciprocalEnvelopeSumBelowLimit`, `CanonicalFermatQuotientThresholdCutoffDiverges`, `Q3SpecialPrimeRaceAbsoluteGapAtLeastTwo`, and `NoLaterUniqueRootConvergentSatisfiesJointNinthOrderCongruences`. All statuses remain `open_not_proven`. Reports: [English](asymmetric-threshold-fixed2adic-head.md) and [한국어](asymmetric-threshold-fixed2adic-head.ko.md).

## Preserved research boundary: TICKET-263

TICKET-263's sharp symmetric reciprocal envelope, existential diagonal Weyl cutoff, mod-32 tie phase, and ninth-order root-cone exactness remain preserved in [English](sharp-envelope-diagonal-mod32-ninthorder.md) and [한국어](sharp-envelope-diagonal-mod32-ninthorder.ko.md).

## Preserved research boundary: TICKET-262

The current machine-readable boundary is
`data/open-problem/ticket262-limsup-finiteharmonic-mod8-thirdorder.json`.
TICKET-262 proves one exact route no-go and three partial theorems while resolving none of the parent conjectures:

- a positive RH-style packet-lag margin is exactly equivalent to the scaled signed-jump limsup being strictly below its positive limit;
- cancellation of any fixed finite set of Weyl harmonics can coexist with positive angular star discrepancy;
- a tie in the special q=3 prime race forces the minus-one count to be `4 mod 8`, while three reproduced actual prefixes have residues `7,1,7`;
- a Twin unit coefficient forces simultaneous third-order congruences modulo `v^3` and `u^3`, and both signs fail jointly on 1,024 certified convergents.

The next single lemmas are the actual-Weil strict scaled-downward-jump limsup bound, vanishing of every nonzero canonical Fermat-quotient Weyl harmonic, exclusion of residue `4 mod 8` for every actual special minus-one count, and exclusion of both third-order congruences on every Twin root convergent. All statuses remain `open_not_proven`. Reports: [English](limsup-finiteharmonic-mod8-thirdorder.md) and [한국어](limsup-finiteharmonic-mod8-thirdorder.ko.md).

## Preserved research boundary: TICKET-261

The canonical artifact is `data/open-problem/ticket261-sharpness-weyl-ties-dualcongruence.json`. It proves two exact route no-gos and two partial theorems while resolving none of the parent conjectures. Reports: [English](sharpness-weyl-ties-dualcongruence.md) and [한국어](sharpness-weyl-ties-dualcongruence.ko.md).
...
TICKET-258 proves two exact route no-gos and two partial theorems while resolving none of the parent conjectures:

- even finite total variation does not repair the RH packet lag obstruction;
- nontrivial phases of distinct odd-prime orders are rationally independent;
- one primitive odd character is a complete reflection detector exactly when `q-1` is a power of two;
- every surviving exponent-17 unit solution is a continued-fraction convergent, and 128 exact convergents exclude denominators through a 62-digit bound.

The next single lemmas are the actual-Weil scaled-variation margin, sublinear canonical Fermat-quotient phase sums, nonzero odd-character moments for every compatible Goldbach prefix, and exclusion of unit coefficients on every Twin root convergent. All statuses remain `open_not_proven`. Reports: [English](variation-character-convergent.md) and [한국어](variation-character-convergent.ko.md).

## Preserved research boundary: TICKET-257
...
TICKET-247 proves two partial theorems and two exact route no-go results while
resolving none of the parent conjectures:

- Hilbert-Schmidt weighted even-moment features have zero coercivity on the
  full normalized even L2 sphere;
- unrestricted Collatz polynomial valuation domination has a bad Hensel
  branch at every prime and arbitrary depth;
- Goldbach rational-center control extends to an arc with the necessary
  `2 pi |beta| M` term, and center-only uniformity is exactly refuted;
- odd composite prime powers have an exact exponent count and a sharper
  square/cube contamination bound.

The next single lemmas are non-Hilbert-Schmidt arithmetic Weil coercivity on
the actual admissible closure; exclusion of actual Fermat quotients from the
bad Hensel branch; uniform signed residue and first-moment saving; and a
scale-local Type-II lower bound beyond the sharp contamination correction.
...
## Preserved research boundary: TICKET-246
...
TICKET-229 remains available in
[English](band-frame-semilinear-character-barriers.md) and
...
## Preserved research boundary: TICKET-215
...
## Preserved research boundary: TICKET-214
...
## Preserved research boundary: TICKET-210
...
## Preserved research boundary: TICKET-206
...
TICKET-200 remains available in
[English](derivative-mesh-three-run-chen-channels.md) and
[한국어](derivative-mesh-three-run-chen-channels.ko.md). TICKET-201 corrects its
fixed-compact RH route and reclassifies its Goldbach/Twin next lemmas.
...
Each problem also includes a `primeproject.ai-proof-forge.v1` object. The forge is the non-reproduction layer: it asks the AI to invent or refine a mathematical object, express the object in a search grammar, run countermodel batteries, and target a formal theorem stub. It also decomposes the next theorem into named sublemmas with roles, risks, proof artifacts, and failure tests, then marks the highest-risk lemma instead of hiding the decisive gap inside prose. For that highest-risk lemma, the forge records a breakthrough object blueprint: the new object family to generate, why it is not a reproduction of known evidence, the AI generation prompt, the minimal counterexample, the falsification oracle, the formalization seed, the success upgrade, and the next experiments. The forge now also includes a counterexample-guided synthesis loop: a candidate schema, forbidden assumptions, oracle pipeline, seed candidates, expected failures, next verifiers, promotion rule, scored ranked candidates, and the top next attack target. That top target is converted into a top attack theorem ticket with a candidate theorem statement, input objects, forbidden premises, first counterexample oracle, required artifact, Lean stub, success condition, and proof attempt protocol. The discovery loop mutates candidate objects, records theorem pressure, runs verifiers, demotes blocked candidates, and keeps only open candidates that still need an infinite-bridge artifact. The loop now has an attack runbook, falsification scorecard, cross-problem synthesis map, ranked portfolio decision, theorem decomposition, breakthrough object blueprint, counterexample-guided synthesis loop, top attack theorem ticket, and next theorem draft mirrored into the Lean-oriented `InfiniteBridge.lean` skeletons. Every candidate must state required outputs, failure exits, pass signals, fail signals, transfer tests, priority scores, stop rules, proof objects, decomposition closure rules, blueprint falsification rules, forbidden assumptions, oracle steps, ranking score, next verifier, theorem-ticket boundary, and ticket protocol before it can be promoted. It explicitly rejects mere replay of known finite checks. 한국어로 말하면, AI proof forge는 이미 알려진 계산을 다시 보여주는 장치가 아니라, 새 수학적 객체를 만들고 그 객체가 실제 장벽을 뚫는지 반례 생성기로 먼저 부숴보는 장치다. 또한 다음 정리를 보조정리 단위로 쪼개서, 어떤 보조정리가 가장 위험하고 어떤 실패 테스트를 통과해야 하는지 공개한다. 최고위험 보조정리에는 breakthrough object blueprint를 붙여서 AI가 생성해야 할 객체군, 최소 반례, 반증 오라클, 형식화 시작점을 바로 추적한다. counterexample-guided synthesis loop는 후보 정리를 만들고, 금지 가정과 반례 오라클로 부수고, 살아남은 후보를 점수화해서 다음 공격 대상으로 올리는 실행 규칙이다. top attack theorem ticket은 이 공격 대상을 검토 가능한 정리 후보로 바꾸되, 아직 증명은 아니라고 명시한다. proof attempt protocol은 그 정리 후보를 어떤 순서로 정의하고, 어떤 산출물을 만들고, 어느 실패 조건에서 폐기할지 지정한다.
