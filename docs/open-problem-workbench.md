...
Each problem also includes a `primeproject.ai-proof-forge.v1` object. The forge is the non-reproduction layer: it asks the AI to invent or refine a mathematical object, express the object in a search grammar, run countermodel batteries, and target a formal theorem stub. It also decomposes the next theorem into named sublemmas with roles, risks, proof artifacts, and failure tests, then marks the highest-risk lemma instead of hiding the decisive gap inside prose. For that highest-risk lemma, the forge records a breakthrough object blueprint: the new object family to generate, why it is not a reproduction of known evidence, the AI generation prompt, the minimal counterexample, the falsification oracle, the formalization seed, the success upgrade, and the next experiments. The forge now also includes a counterexample-guided synthesis loop: a candidate schema, forbidden assumptions, oracle pipeline, seed candidates, expected failures, next verifiers, promotion rule, scored ranked candidates, and the top next attack target. The discovery loop mutates candidate objects, records theorem pressure, runs verifiers, demotes blocked candidates, and keeps only open candidates that still need an infinite-bridge artifact. The loop now has an attack runbook, falsification scorecard, cross-problem synthesis map, ranked portfolio decision, theorem decomposition, breakthrough object blueprint, counterexample-guided synthesis loop, and next theorem draft mirrored into the Lean-oriented `InfiniteBridge.lean` skeletons. Every candidate must state required outputs, failure exits, pass signals, fail signals, transfer tests, priority scores, stop rules, proof objects, decomposition closure rules, blueprint falsification rules, forbidden assumptions, oracle steps, ranking score, and next verifier before it can be promoted. It explicitly rejects mere replay of known finite checks. 한국어로 말하면, AI proof forge는 이미 알려진 계산을 다시 보여주는 장치가 아니라, 새 수학적 객체를 만들고 그 객체가 실제 장벽을 뚫는지 반례 생성기로 먼저 부숴보는 장치다. 또한 다음 정리를 보조정리 단위로 쪼개서, 어떤 보조정리가 가장 위험하고 어떤 실패 테스트를 통과해야 하는지 공개한다. 최고위험 보조정리에는 breakthrough object blueprint를 붙여서 AI가 생성해야 할 객체군, 최소 반례, 반증 오라클, 형식화 시작점을 바로 추적한다. counterexample-guided synthesis loop는 후보 정리를 만들고, 금지 가정과 반례 오라클로 부수고, 살아남은 후보를 점수화해서 다음 공격 대상으로 올리는 실행 규칙이다.
