...
```text
Next: ArchimedeanNaturalExclusionForAllInfiniteCapPaths
다음: 모든 무한 cap 경로의 양의 자연수 궤도 실현을 배제하는 Archimedean 정리
```

The surviving nested words define 2-adic paths. Positive integers are dense in
the 2-adics, so only a changing-precision argument coupled to ordinary integer
magnitude can close the universal gap. Collatz remains open.
...
```text
Next: VaughanRelativeIncrementImbalanceBelow2Over23
다음: 실제 Vaughan 상대 증분 불균형의 limsup 2/23 미만 정리
```

No such estimate is proved for the actual coefficients. Parity and exact
...
|---|---|---|---|
| RH | certified interval enumeration on the rational-bump core | finite non-discovery as positivity | `UniversalNonnegativeWeilCoreCertificate` |
| Collatz | exponential rarity plus changing-precision Archimedean control | finite cap-language extinction | `ArchimedeanNaturalExclusionForAllInfiniteCapPaths` |
| Goldbach | pointwise signed residual target `K=56` | more constant polishing at fixed `H` | `PointwiseBinaryGoldbachResidualK56` |
| Twin Prime | dimensionless relative increment imbalance | detached endpoint interpolation | `VaughanRelativeIncrementImbalanceBelow2Over23` |
