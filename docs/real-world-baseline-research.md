...
감사 기록을 파일로 남기려면 `--report publication_reproduction_report.json`을 붙인다. report에는 실행 명령, 공개 artifact 경로, 재생성 SHA-256, 공개 SHA-256, JSON 동등성, byte 동등성, JSON mismatch, byte mismatch가 들어간다. 실행마다 달라지는 임시 출력 경로는 `{tmp}`로 정규화된다.
