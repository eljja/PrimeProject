...
`required_evidence`도 같은 경계를 따른다. controlled synthetic classifier vector는 classifier plumbing과 합성 신호 검증에는 유용하지만, `real_world_claim_ready=true`인 labelled vector가 아니면 `real_world_labelled_feature_vectors`는 `missing`으로 남긴다.
...
감사 기록을 파일로 남기려면 `--report publication_reproduction_report.json`을 붙인다. report에는 실행 명령, 공개 artifact 경로, 재생성 SHA-256, 공개 SHA-256, JSON 동등성, byte 동등성, JSON mismatch, byte mismatch가 들어간다. 실행마다 달라지는 임시 출력 경로는 `{tmp}`로 정규화된다. `publication_reproduction_report*.json`은 로컬 감사 scratch output으로 취급해 git ignore 대상에 둔다.
