...
> **TICKET-130 correction / 정정:** the proposed Collatz next target
> `LeastCounterexampleValuationCapLanguageExtinction` is retired. The mechanical
> word `a_j=C_(j+1)-C_j` survives every finite depth and every finite prefix has
> infinitely many positive realizers by TICKET-78. See
> [TICKET-130](computability-cap-language-optimality.md). TICKET-129's proved
> valuation cap remains valid; only its proposed next target was incorrect.
>
> **한국어:** TICKET-129의 Collatz valuation 상한 정리 자체는 유효하지만,
> 이후 제안한 “유한 cap 언어 소멸” 목표는 TICKET-130에서 불가능함이
> 증명되어 폐기되었다.
>
> **TICKET-131 scope correction / 범위 정정:** this strict cap is a
> least-counterexample necessity only for `j<=2^29`; it is not established as
> an all-time condition. TICKET-131 replaces the later infinite strict-cap target with an
> exact nested-residue stabilization contract.

> **TICKET-132 admissibility correction / 허용공간 정정:** the unconstrained
> rational-bump core is dense in `C_c^infinity(R)`, but its generic elements do
> not satisfy the two exact Weil moment conditions. TICKET-132 applies an
> explicit continuous two-anchor projection and obtains a countable effective
> core dense inside the moment-constrained subspace. The density theorem here
> remains valid; only its direct use as an admissible Weil core is narrowed.
...
This historical target is superseded by TICKET-130. Compatible finite words do
not merely survive the current audit: an explicit word survives every finite
depth. TICKET-130 first replaced it with
`ArchimedeanNaturalExclusionForAllInfiniteCapPaths`; TICKET-131 then showed
that this was still insufficient because the strict cap has only a finite
proved horizon. The current target is
`NoEventuallyStableNaturalPathUnderExactNoDescentEnvelope`. No contradiction
and no Collatz proof has been obtained.
