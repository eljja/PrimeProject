...
| `docs/commoncore-baker-angle-typeii.md` | latest four-conjecture exact-reduction and no-go report | four reproducible common-core/Baker/angle/Type-II results, including eventual descent for one explicit Collatz family | proof or counterexample for any target conjecture |
...
| `docs/core-eigen-first-crossing-pointwise-product.md` | 최신 네 난제 target 교정 및 no-go 보고서 | 재현 가능한 constraint-core/first-crossing/pointwise/product-Haar 결과 4건 | 어떤 대상 추측의 증명 또는 반례 |
...
| `docs/commoncore-baker-angle-typeii.md` | 최신 네 난제 정확 환원 및 no-go 보고서 | 재현 가능한 common-core/Baker/angle/Type-II 결과 4건과 명시적 Collatz 한 가족의 점근 하강 | 어떤 대상 추측의 증명 또는 반례 |
