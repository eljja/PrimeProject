...
async function main() {
  ...
  const ticket234EarlyRender = openProblemSource.indexOf("render(payload, problem);", ticket234Load);
  ...
  for (const page of ["riemann", "collatz", "goldbach", "twin-prime"]) {
    ...
    if (!source.includes("open-problems.js?v=20260822-ticket234")) {
      errors.push(`${page}: missing evidence-first proof-page cache key`);
    }
    if (!source.includes("styles.css?v=20260822-ticket234-layout")) {
  ...
  if (
    ...
    !metrics.currentBriefText.includes("TICKET-234") ||
    !metrics.currentBriefText.includes("ArithmeticWeilTailKernelCompatibilityAndPositiveSchurComplement") ||
    !metrics.currentBriefText.includes("UniformBinaryDensityBandAdaptiveRadicalDeficit") ||
    !metrics.currentBriefText.includes("ComplementaryHalfPrimeReflectionMinorCoherenceAtInverseLogScale") ||
    !metrics.currentBriefText.includes("PrimeWeightedFixedDegreeCesaroCRTCorrelationDecayAtTwinScale") ||
    !metrics.currentBriefText.includes("0 / 4 resolved") ||
    !metrics.currentBriefText.includes("OpenSSL") ||
    metrics.railResearchTicket.trim() !== "TICKET-234" ||
    !metrics.railResearchText.includes("TICKET-234") ||
  ...
  const proofHubChecks = [
    ...
    ["TICKET-234 boundary", metrics.proofHub?.boundary.includes("What TICKET-234 actually changed")],
    ["Riemann next lemma", metrics.proofHub?.boundary.includes("ArithmeticWeilTailKernelCompatibilityAndPositiveSchurComplement")],
    ["Collatz next lemma", metrics.proofHub?.boundary.includes("UniformBinaryDensityBandAdaptiveRadicalDeficit")],
    ["Goldbach next lemma", metrics.proofHub?.boundary.includes("ComplementaryHalfPrimeReflectionMinorCoherenceAtInverseLogScale")],
    ["Twin next lemma", metrics.proofHub?.boundary.includes("PrimeWeightedFixedDegreeCesaroCRTCorrelationDecayAtTwinScale")],
    ["progression range", metrics.proofHub?.boundary.includes("TICKET-161–234")],
  ...
  const invalidProofOrganization = metrics.openProblemPages.flatMap((page) => {
    ...
    if (!page.currentBoundaryLabel.includes("TICKET-234 · CURRENT RESEARCH BOUNDARY")) failures.push(`${page.problemId}: static TICKET-234 boundary label missing`);
    if (!page.currentResearchText.includes("Ticket 234 operator kernels, binary affine-sieve no-go, Goldbach half-channels, and Poisson CRT noise")) failures.push(`${page.problemId}: current TICKET-234 boundary missing`);
  ...
  const missingTicket71Checks = metrics.openProblemPages.flatMap((page) => {
    ...
    requireCurrentText("ticket234 title", "Ticket 234 operator kernels, binary affine-sieve no-go, Goldbach half-channels, and Poisson CRT noise");
    requireCurrentText("ticket234 table", "TICKET234 audit");
    requireCurrentText("ticket234 latest", "LATEST / 최신 연구 경계");
    requireCurrentText("ticket234 next lemmas", "Next lemmas4");
    requireCurrentText("ticket234 resolutions", "Resolution count0");
    requireCurrentText("ticket234 proof DAG", "Proof DAG / 증명 의존성");
    if (page.ticket234AuditOverflow) checks.push(`${page.problemId}: ticket234 audit table overflow`);
    if (page.problemId === "riemann") {
      requireCurrentText("ticket234 RH theorem", "ScalarDiagonalFrameRankAndSignedTailTransferNoGo");
      requireCurrentText("ticket234 RH target", "ArithmeticWeilTailKernelCompatibilityAndPositiveSchurComplement");
      requireCurrentText("ticket234 RH rank", "Scalar floor + singular Gramproved");
      requireCurrentText("ticket234 RH no-go", "Size-only transferrefuted");
      requireCurrentText("ticket234 RH open", "Arithmetic kernel compatibilityopen");
    } else if (page.problemId === "collatz") {
      requireCurrentText("ticket234 Collatz theorem", "UniformBinaryDensityBandFixedFiniteAffineSieveNoGo");
      requireCurrentText("ticket234 Collatz target", "UniformBinaryDensityBandAdaptiveRadicalDeficit");
      requireCurrentText("ticket234 Collatz count", "Fixed moduli checked66");
      requireCurrentText("ticket234 Collatz no-go", "Infinite false-positive familyproved");
      requireCurrentText("ticket234 Collatz open", "Adaptive radical deficitopen");
    } else if (page.problemId === "goldbach") {
      requireCurrentText("ticket234 Goldbach theorem", "MinorArcMarginGoldbachEquivalenceAndPrimeHalfChannelCancellationNoGo");
      requireCurrentText("ticket234 Goldbach target", "ComplementaryHalfPrimeReflectionMinorCoherenceAtInverseLogScale");
      requireCurrentText("ticket234 Goldbach endpoint", "Full strict minor marginendpoint-equivalent");
      requireCurrentText("ticket234 Goldbach cancellation", "Half-channel cancellationexact");
      requireCurrentText("ticket234 Goldbach open", "Inverse-log cross coherenceopen");
    } else {
      requireCurrentText("ticket234 Twin theorem", "PoissonizedFixedDegreeCesaroCriterionAndMovingPrimeNoGo");
      requireCurrentText("ticket234 Twin target", "PrimeWeightedFixedDegreeCesaroCRTCorrelationDecayAtTwinScale");
      requireCurrentText("ticket234 Twin criterion", "Fixed-degree criterioniff proved");
      requireCurrentText("ticket234 Twin no-go", "Fixed-labelled routerefuted");
      requireCurrentText("ticket234 Twin open", "Actual prime Cesàro decayopen");
    }
