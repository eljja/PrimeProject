...
async function main() {
  ...
  const ticket233EarlyRender = openProblemSource.indexOf("render(payload, problem);", ticket233Load);
  ...
  for (const page of ["riemann", "collatz", "goldbach", "twin-prime"]) {
    ...
    if (!source.includes("open-problems.js?v=20260821-ticket233")) {
      errors.push(`${page}: missing evidence-first proof-page cache key`);
    }
    if (!source.includes("styles.css?v=20260821-ticket233-layout")) {
    ...
    for (const [problemId, href] of [
      ...
      metrics.openProblemPages.push(
        await problemPage.evaluate((expectedProblemId) => ({
          ...
          ticket233AuditOverflow: (() => {
            const wrapper = document.querySelector(
              "#ticket233-logarithmic-frame-density-shell-entropy .ticket161-audit-table .proof-table-wrap",
            );
  ...
  if (
    ...
    !metrics.currentBriefText.includes("TICKET-233") ||
    !metrics.currentBriefText.includes("LogarithmicAdaptiveScalarFrameToWeilKernelTransferWithExplicitSignedTailDominance") ||
    !metrics.currentBriefText.includes("UniformBinaryDensityBandPrimitiveNecklaceNondivisibility") ||
    !metrics.currentBriefText.includes("UniformTargetAlignedBinaryPrimeMinorArcNegativeMassBelowPolylogMajorArcMargin") ||
    !metrics.currentBriefText.includes("PrimeWeightedCriticalNoiseCRTChiSquareDecayAtTwinScale") ||
    !metrics.currentBriefText.includes("0 / 4 resolved") ||
    !metrics.currentBriefText.includes("OpenSSL") ||
    metrics.railResearchTicket.trim() !== "TICKET-233" ||
    !metrics.railResearchText.includes("TICKET-233") ||
  ...
  const proofHubChecks = [
    ...
    ["TICKET-233 boundary", metrics.proofHub?.boundary.includes("What TICKET-233 actually changed")],
    ["Riemann next lemma", metrics.proofHub?.boundary.includes("LogarithmicAdaptiveScalarFrameToWeilKernelTransferWithExplicitSignedTailDominance")],
    ["Collatz next lemma", metrics.proofHub?.boundary.includes("UniformBinaryDensityBandPrimitiveNecklaceNondivisibility")],
    ["Goldbach next lemma", metrics.proofHub?.boundary.includes("UniformTargetAlignedBinaryPrimeMinorArcNegativeMassBelowPolylogMajorArcMargin")],
    ["Twin next lemma", metrics.proofHub?.boundary.includes("PrimeWeightedCriticalNoiseCRTChiSquareDecayAtTwinScale")],
    ["progression range", metrics.proofHub?.boundary.includes("TICKET-161–233")],
  ...
  const invalidProofOrganization = metrics.openProblemPages.flatMap((page) => {
    ...
    if (!page.currentBoundaryLabel.includes("TICKET-233 · CURRENT RESEARCH BOUNDARY")) failures.push(`${page.problemId}: static TICKET-233 boundary label missing`);
    if (!page.currentResearchText.includes("Ticket 233 logarithmic frames, Collatz twelve-one closure, squarefree Goldbach shells, and CRT entropy")) failures.push(`${page.problemId}: current TICKET-233 boundary missing`);
  ...
  const missingTicket71Checks = metrics.openProblemPages.flatMap((page) => {
    ...
    requireCurrentText("ticket233 title", "Ticket 233 logarithmic frames, Collatz twelve-one closure, squarefree Goldbach shells, and CRT entropy");
    requireCurrentText("ticket233 table", "TICKET233 audit");
    requireCurrentText("ticket233 latest", "LATEST / 최신 연구 경계");
    requireCurrentText("ticket233 next lemmas", "Next lemmas4");
    requireCurrentText("ticket233 resolutions", "Resolution count0");
    requireCurrentText("ticket233 proof DAG", "Proof DAG / 증명 의존성");
    if (page.ticket233AuditOverflow) checks.push(`${page.problemId}: ticket233 audit table overflow`);
    if (page.problemId === "riemann") {
      requireCurrentText("ticket233 RH theorem", "LogarithmicAdaptiveScalarFrameExistenceAndSharpDimensionThreshold");
      requireCurrentText("ticket233 RH target", "LogarithmicAdaptiveScalarFrameToWeilKernelTransferWithExplicitSignedTailDominance");
      requireCurrentText("ticket233 RH threshold", "Scalar thresholdΘ(log T)");
      requireCurrentText("ticket233 RH no-go", "Superlog necessityrefuted");
      requireCurrentText("ticket233 RH open", "Weil transferopen");
    } else if (page.problemId === "collatz") {
      requireCurrentText("ticket233 Collatz theorem", "BinaryLineageCorrectionTwelveOneExclusionAndFixedStratumNoGo");
      requireCurrentText("ticket233 Collatz target", "UniformBinaryDensityBandPrimitiveNecklaceNondivisibility");
      requireCurrentText("ticket233 Collatz represented", "k=12 represented28,729,599,990");
      requireCurrentText("ticket233 Collatz binary lower", "Binary remaining k≥13");
      requireCurrentText("ticket233 Collatz general lower", "General remaining k≥8");
    } else if (page.problemId === "goldbach") {
      requireCurrentText("ticket233 Goldbach theorem", "PolylogarithmicSquarefreePrimeShellAsymptoticAndSparseDenominatorNoGo");
      requireCurrentText("ticket233 Goldbach target", "UniformTargetAlignedBinaryPrimeMinorArcNegativeMassBelowPolylogMajorArcMargin");
      requireCurrentText("ticket233 Goldbach theorem flag", "Polylog squarefree shellproved");
      requireCurrentText("ticket233 Goldbach no-go", "Unrestricted growthrefuted");
      requireCurrentText("ticket233 Goldbach open", "Minor arcsopen");
    } else {
      requireCurrentText("ticket233 Twin theorem", "CriticalEntropyDampedSignedCRTLargeSieveAndParityRetentionNoGo");
      requireCurrentText("ticket233 Twin target", "PrimeWeightedCriticalNoiseCRTChiSquareDecayAtTwinScale");
      requireCurrentText("ticket233 Twin bound", "Damped large sieveproved");
      requireCurrentText("ticket233 Twin no-go", "Critical centering routerefuted");
      requireCurrentText("ticket233 Twin open", "Positive twin mainopen");
    }
