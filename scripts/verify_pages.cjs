...
async function main() {
  ...
  const ticket212EarlyRender = openProblemSource.indexOf("render(payload, problem);", ticket212Load);
  ...
  for (const page of ["riemann", "collatz", "goldbach", "twin-prime"]) {
    ...
    if (!source.includes("open-problems.js?v=20260812-ticket212")) {
      errors.push(`${page}: missing evidence-first proof-page cache key`);
    }
    if (!source.includes("styles.css?v=20260812-ticket212-layout")) {
    ...
    for (const [problemId, href] of [
      ...
      metrics.openProblemPages.push(
        await problemPage.evaluate((expectedProblemId) => ({
          ...
          ticket212AuditOverflow: (() => {
            const wrapper = document.querySelector(
              "#ticket212-even-defect-ghost-bonferroni-gapchannel .ticket161-audit-table .proof-table-wrap",
            );
  ...
  if (
    ...
    !metrics.currentBriefText.includes("TICKET-212") ||
    !metrics.currentBriefText.includes("전체 증인 곱") ||
    !metrics.currentBriefText.includes("2-adic 유령") ||
    !metrics.currentBriefText.includes("간격 2") ||
  ...
  if (
    ...
    !metrics.proofHub?.boundary.includes("What TICKET-212 actually changed") ||
  ...
  const invalidProofOrganization = metrics.openProblemPages.flatMap((page) => {
    ...
    if (!page.currentBoundaryLabel.includes("TICKET-212 · CURRENT RESEARCH BOUNDARY")) failures.push(`${page.problemId}: static TICKET-212 boundary label missing`);
    if (!page.currentResearchText.includes("Ticket 212 even defect")) failures.push(`${page.problemId}: current TICKET-212 boundary missing`);
  ...
  const missingTicket71Checks = metrics.openProblemPages.flatMap((page) => {
    ...
    requireCurrentText("ticket212 title", "Ticket 212 even defect, 2-adic ghosts, full-witness products, and gap channels");
    requireCurrentText("ticket212 table", "TICKET212 audit");
    requireCurrentText("ticket212 latest", "LATEST / 최신 연구 경계");
    requireCurrentText("ticket212 exact results", "Exact results4");
    requireCurrentText("ticket212 resolutions", "Resolution count0");
    requireCurrentText("ticket212 proof DAG", "Proof DAG / 증명 의존성");
    if (page.ticket212AuditOverflow) checks.push(`${page.problemId}: ticket212 audit table overflow`);
    if (page.problemId === "riemann") {
      requireCurrentText("ticket212 RH theorem", "EvenCriticalLineDefectSubTwoSaturationCertificate");
      requireCurrentText("ticket212 RH target", "UniformAllHeightCriticalLineDefectStrictlyBelowTwo");
      requireCurrentText("ticket212 RH certificate", "Sub-two certificateproved");
      requireCurrentText("ticket212 RH sharpness", "Threshold 2sharp");
      requireCurrentText("ticket212 RH all-height", "Actual all-height boundopen");
    } else if (page.problemId === "collatz") {
      requireCurrentText("ticket212 Collatz theorem", "TwoAdicGhostUniversalityAndOddDivisibilityCorrection");
      requireCurrentText("ticket212 Collatz target", "UniformOddDivisorNondivisibilityForHighOneDensityWords");
      requireCurrentText("ticket212 Collatz ghosts", "Ghost universalityproved");
      requireCurrentText("ticket212 Collatz no-go", "Z2 obstructionrefuted");
      requireCurrentText("ticket212 Collatz divisibility", "Odd divisibilityopen");
    } else if (page.problemId === "goldbach") {
      requireCurrentText("ticket212 Goldbach theorem", "FullWitnessProductIdentityAndFixedBonferroniNoGo");
      requireCurrentText("ticket212 Goldbach target", "UniformFullWitnessProductResummationBelowOne");
      requireCurrentText("ticket212 Goldbach product", "Full witness productproved");
      requireCurrentText("ticket212 Goldbach no-go", "Fixed Bonferronirefuted");
      requireCurrentText("ticket212 Goldbach multiplicity", "max A lower bound");
      requireCurrentText("ticket212 Goldbach resummation", "Uniform resummationopen");
    } else {
      requireCurrentText("ticket212 Twin theorem", "DyadicGapTwoEquivalenceAndFiniteGapAggregateNoGo");
      requireCurrentText("ticket212 Twin target", "GapTwoDyadicChannelPositiveOnInfinitelyManyBlocks");
      requireCurrentText("ticket212 Twin equivalence", "Dyadic equivalenceproved");
      requireCurrentText("ticket212 Twin no-go", "Aggregate selects gap 2refuted");
      requireCurrentText("ticket212 Twin blocks", "Infinite gap-2 blocksopen");
    }
