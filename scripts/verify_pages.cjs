...
async function main() {
  ...
  for (const page of ["riemann", "collatz", "goldbach", "twin-prime"]) {
    ...
    if (!source.includes("open-problems.js?v=20260826-ticket246") || !source.includes("ticket246-open-problem.js?v=20260826-ticket246") || !source.includes("ticket245-open-problem.js?v=20260826-ticket245") || !source.includes("ticket244-open-problem.js?v=20260826-ticket244") || !source.includes("ticket243-open-problem.js?v=20260826-ticket243") || !source.includes("ticket242-open-problem.js?v=20260825-ticket242")) {
      errors.push(`${page}: missing evidence-first proof-page cache key`);
    }
    if (!source.includes("styles.css?v=20260826-ticket246-layout")) {
    ...
    for (const [problemId, href] of [
      ...
      metrics.openProblemPages.push(
        await problemPage.evaluate((expectedProblemId) => ({
          ...
          ticket246AuditOverflow: (() => {
            const wrapper = document.querySelector(
              "#ticket246-moment-alldepth-parseval-primepower .ticket246-audit-table .proof-table-wrap",
            );
  ...
  if (
    ...
    !metrics.currentBriefText.includes("TICKET-246") ||
    !metrics.currentBriefText.includes("InfiniteFeatureCoercivityOnNormalizedAdmissibleWeilClosure") ||
    !metrics.currentBriefText.includes("FixedBaseAllPrimeValuationDominationForPqByUqMinusVq") ||
    !metrics.currentBriefText.includes("UniformQuarterTorusResidueVarianceDecayWithArcStability") ||
    !metrics.currentBriefText.includes("ScaleLocalTypeIILowerBoundBeyondPrimePowerContamination") ||
    !metrics.currentBriefText.includes("0 / 4 resolved") ||
    !metrics.currentBriefText.includes("OpenSSL") ||
    metrics.railResearchTicket.trim() !== "TICKET-246" ||
    !metrics.railResearchText.includes("TICKET-246") ||
  ...
  const proofHubChecks = [
    ...
    ["TICKET-246 boundary", metrics.proofHub?.boundary.includes("What TICKET-246 actually changed")],
    ["Riemann next lemma", metrics.proofHub?.boundary.includes("InfiniteFeatureCoercivityOnNormalizedAdmissibleWeilClosure")],
    ["Collatz next lemma", metrics.proofHub?.boundary.includes("FixedBaseAllPrimeValuationDominationForPqByUqMinusVq")],
    ["Goldbach next lemma", metrics.proofHub?.boundary.includes("UniformQuarterTorusResidueVarianceDecayWithArcStability")],
    ["Twin next lemma", metrics.proofHub?.boundary.includes("ScaleLocalTypeIILowerBoundBeyondPrimePowerContamination")],
    ["progression range", metrics.proofHub?.boundary.includes("TICKET-161–246")],
  ...
  const invalidProofOrganization = metrics.openProblemPages.flatMap((page) => {
    ...
    if (!page.currentBoundaryLabel.includes("TICKET-246 · CURRENT RESEARCH BOUNDARY")) failures.push(`${page.problemId}: static TICKET-246 boundary label missing`);
    if (!page.currentResearchText.includes("Ticket 246 finite-moment annihilators, all-depth Fermat polynomials, rational-center Parseval, and prime-power contamination")) failures.push(`${page.problemId}: current TICKET-246 boundary missing`);
  ...
  const missingTicket71Checks = metrics.openProblemPages.flatMap((page) => {
    ...
    requireCurrentText("ticket246 title", "Ticket 246 finite-moment annihilators, all-depth Fermat polynomials, rational-center Parseval, and prime-power contamination");
    requireCurrentText("ticket246 table", "TICKET246 audit");
    requireCurrentText("ticket246 latest", "LATEST / 최신 연구 경계");
    requireCurrentText("ticket246 resolutions", "Resolution count0");
    requireCurrentText("ticket246 proof DAG", "Proof DAG / 증명 의존성");
    requireCurrentText("ticket246 completion guard", "TICKET-246 resolves none of the four parent conjectures");
    if (page.ticket246AuditOverflow) checks.push(`${page.problemId}: ticket246 audit table overflow`);
    if (page.problemId === "riemann") {
      requireCurrentText("ticket246 RH theorem", "FiniteEvenMomentAnnihilatorNoGo");
      requireCurrentText("ticket246 RH target", "InfiniteFeatureCoercivityOnNormalizedAdmissibleWeilClosure");
      requireCurrentText("ticket246 RH no-go", "Finite-moment separationrefuted");
      requireCurrentText("ticket246 RH open", "Genuine Weil coercivityopen");
    } else if (page.problemId === "collatz") {
      requireCurrentText("ticket246 Collatz theorem", "AllDepthFixedBaseFermatPolynomialIdentity");
      requireCurrentText("ticket246 Collatz target", "FixedBaseAllPrimeValuationDominationForPqByUqMinusVq");
      requireCurrentText("ticket246 Collatz replay", "Primes replayed17,981");
      requireCurrentText("ticket246 Collatz identity", "All-depth identityproved");
      requireCurrentText("ticket246 Collatz open", "All-prime dominationopen");
    } else if (page.problemId === "goldbach") {
      requireCurrentText("ticket246 Goldbach theorem", "RationalCenterResidueParsevalBridge");
      requireCurrentText("ticket246 Goldbach target", "UniformQuarterTorusResidueVarianceDecayWithArcStability");
      requireCurrentText("ticket246 Goldbach Parseval", "Center Parsevalproved");
      requireCurrentText("ticket246 Goldbach decay", "Growing-q decayopen");
      requireCurrentText("ticket246 Goldbach open", "Arc stabilityopen");
    } else {
      requireCurrentText("ticket246 Twin theorem", "PrimePowerPairProxyContaminationBound");
      requireCurrentText("ticket246 Twin target", "ScaleLocalTypeIILowerBoundBeyondPrimePowerContamination");
      requireCurrentText("ticket246 Twin bound", "Contamination boundproved");
      requireCurrentText("ticket246 Twin no-go", "Uncorrected equalityrefuted");
      requireCurrentText("ticket246 Twin open", "Scale-local Type IIopen");
    }
  ...
  if (
    ...
    !metrics.atlasPanel.includes("TICKET-246") ||
    !metrics.atlasPanel.includes("InfiniteFeatureCoercivityOnNormalizedAdmissibleWeilClosure") ||
