...
async function main() {
  ...
  const ticket213EarlyRender = openProblemSource.indexOf("render(payload, problem);", ticket213Load);
  ...
  for (const page of ["riemann", "collatz", "goldbach", "twin-prime"]) {
    ...
    if (!source.includes("open-problems.js?v=20260812-ticket213")) {
      errors.push(`${page}: missing evidence-first proof-page cache key`);
    }
    if (!source.includes("styles.css?v=20260812-ticket213-layout")) {
    ...
    for (const [problemId, href] of [
      ...
      metrics.openProblemPages.push(
        await problemPage.evaluate((expectedProblemId) => ({
          ...
          ticket213AuditOverflow: (() => {
            const wrapper = document.querySelector(
              "#ticket213-multiplicity-sixone-polynomial-selector .ticket161-audit-table .proof-table-wrap",
            );
  ...
  if (
    ...
    !metrics.currentBriefText.includes("TICKET-213") ||
    !metrics.currentBriefText.includes("376,788") ||
    !metrics.currentBriefText.includes("고정 실수 다항식") ||
  ...
  if (
    ...
    !metrics.proofHub?.boundary.includes("What TICKET-213 actually changed") ||
  ...
  const invalidProofOrganization = metrics.openProblemPages.flatMap((page) => {
    ...
    if (!page.currentBoundaryLabel.includes("TICKET-213 · CURRENT RESEARCH BOUNDARY")) failures.push(`${page.problemId}: static TICKET-213 boundary label missing`);
    if (!page.currentResearchText.includes("Ticket 213 multiplicity-aware counts")) failures.push(`${page.problemId}: current TICKET-213 boundary missing`);
  ...
  const missingTicket71Checks = metrics.openProblemPages.flatMap((page) => {
    ...
    requireCurrentText("ticket213 title", "Ticket 213 multiplicity-aware counts, six-one cycles, polynomial majorants, and gap selectors");
    requireCurrentText("ticket213 table", "TICKET213 audit");
    requireCurrentText("ticket213 latest", "LATEST / 최신 연구 경계");
    requireCurrentText("ticket213 exact results", "Exact results4");
    requireCurrentText("ticket213 resolutions", "Resolution count0");
    requireCurrentText("ticket213 proof DAG", "Proof DAG / 증명 의존성");
    if (page.ticket213AuditOverflow) checks.push(`${page.problemId}: ticket213 audit table overflow`);
    if (page.problemId === "riemann") {
      requireCurrentText("ticket213 RH theorem", "MultiplicityAwareCriticalLineCountEquivalenceAndSignChangeNoGo");
      requireCurrentText("ticket213 RH target", "UniformMultiplicityAwareCriticalLineDefectStrictlyBelowTwo");
      requireCurrentText("ticket213 RH equivalence", "Multiplicity RH equivalenceproved");
      requireCurrentText("ticket213 RH no-go", "Sign count equals RHrefuted");
      requireCurrentText("ticket213 RH all-height", "Actual all-height boundopen");
    } else if (page.problemId === "collatz") {
      requireCurrentText("ticket213 Collatz theorem", "CompleteSixValuationOneCycleStratumExclusion");
      requireCurrentText("ticket213 Collatz target", "UniformPrimitiveOddDivisorWitnessForAllOneCountsAtLeastSeven");
      requireCurrentText("ticket213 Collatz words", "Exact words376,788");
      requireCurrentText("ticket213 Collatz exclusion", "Six-one stratumexcluded");
      requireCurrentText("ticket213 Collatz minimum", "Minimum ones in a cycle7");
    } else if (page.problemId === "goldbach") {
      requireCurrentText("ticket213 Goldbach theorem", "FixedDegreePolynomialWitnessMajorantNoGo");
      requireCurrentText("ticket213 Goldbach target", "ScaleGrowingWitnessResummationWithUniformDyadicTailBelowOne");
      requireCurrentText("ticket213 Goldbach no-go", "Fixed polynomial routerefuted");
      requireCurrentText("ticket213 Goldbach degree", "Degree ≥ witness rangeproved");
      requireCurrentText("ticket213 Goldbach tail", "Uniform subunit tailopen");
    } else {
      requireCurrentText("ticket213 Twin theorem", "NonnegativeGapFunctionalIsolationIffSupportAtTwo");
      requireCurrentText("ticket213 Twin target", "GapTwoSelectiveSignedFunctionalWithUniformArithmeticRemainder");
      requireCurrentText("ticket213 Twin equivalence", "Selector characterizationproved");
      requireCurrentText("ticket213 Twin no-go", "Contaminated aggregatesrefuted");
      requireCurrentText("ticket213 Twin signed", "Signed arithmetic selectoropen");
    }
