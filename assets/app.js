  ...
  change_dashboard: {
    ...
    visual_rollup: {
      ...
      evidence_spine: [
        ...
        { layer: "Open-Proof", score: 99, status: "open", artifacts: ["data/open-problem/ticket246-moment-alldepth-parseval-primepower.json", "docs/moment-alldepth-parseval-primepower.md", "docs/moment-alldepth-parseval-primepower.ko.md", "docs/proof-or-counterexample-program.md"], gate: "TICKET-246 proves three partial theorems and one exact route no-go, records four acyclic proof DAGs with zero machine failures, checks nine moment annihilators, replays five q-adic digits for 17,981 primes, audits 186 Goldbach denominator cases, and checks six twin proxy scales while keeping every resolution counter at zero; score 99 measures workflow coverage, not proof probability or conjecture completion.", proof: "The workbench now requires infinite-feature coercivity on the genuine Weil closure, all-prime valuation domination for Pq versus Uq−Vq, growing-denominator residue-variance decay with arc stability, and a scale-local Type-II lower bound beyond prime-power contamination." },
      ],
    },
    latest_changes: [
      { label: "Evidence-first Pages", impact: "Public entry points now lead with TICKET-246: finite-moment annihilation, all-depth Fermat algebra, rational-center Parseval, and prime-power contamination, while preserving TICKET-245 and the full ledger.", metric: "4 current cards / 246 tickets" },
    ...
    openProblems: [
      {
        ...
        route: "TICKET-246 exact no-go: every fixed finite list of even moments has an explicit normalized compact-support annihilator in the model class. / 고정된 유한 짝수 모멘트 목록마다 명시적 정규화 유한지지 소거자가 있습니다.",
        artifact: "next: InfiniteFeatureCoercivityOnNormalizedAdmissibleWeilClosure",
      },
      {
        name: "Collatz Conjecture / 콜라츠 추측",
        href: "open-problems/collatz.html",
        status: "open_not_proven / 미해결",
        route: "TICKET-246 partial: one exact terminating polynomial decides every q-adic depth for the fixed 32/27 difference; the all-prime valuation comparison remains open. / 끝나는 정확 다항식이 고정 밑 차의 모든 q진 깊이를 판정하지만 전소수 valuation 비교는 열려 있습니다.",
        artifact: "next: FixedBaseAllPrimeValuationDominationForPqByUqMinusVq",
      },
      {
        name: "Strong Goldbach / 강한 골드바흐 추측",
        href: "open-problems/goldbach.html",
        status: "open_not_proven / 미해결",
        route: "TICKET-246 partial: rational-center residual energy is exactly q times the prime-residue discrepancy square sum. / 유리 중심 잔차 에너지는 소수 잉여류 discrepancy 제곱합의 정확히 q배입니다.",
        artifact: "next: UniformQuarterTorusResidueVarianceDecayWithArcStability",
      },
      {
        name: "Twin Prime Conjecture / 쌍둥이 소수 추측",
        href: "open-problems/twin-prime.html",
        status: "open_not_proven / 미해결",
        route: "TICKET-246 partial: the odd prime-power pair proxy has explicit composite-power contamination, and the uncorrected equality fails first at (7,9). / 홀수 소수거듭제곱 쌍 프록시는 명시적 오염을 가지며 무보정 동일시는 (7,9)에서 처음 실패합니다.",
        artifact: "next: ScaleLocalTypeIILowerBoundBeyondPrimePowerContamination",
      },
    ],    nextSteps: [
      ...openGaps.map((gap) => ({
