  ...
  change_dashboard: {
    ...
    visual_rollup: {
      ...
      evidence_spine: [
        ...
        { layer: "Open-Proof", score: 99, status: "open", artifacts: ["data/open-problem/ticket239-cancellation-lifting-fourier-crt.json", "docs/cancellation-lifting-fourier-crt.md", "docs/cancellation-lifting-fourier-crt.ko.md", "docs/proof-or-counterexample-program.md"], gate: "TICKET-239 proves four exact partial or no-go theorems, records four proof DAGs with zero machine failures, and keeps all four parent-conjecture resolution counters at zero; score 99 measures workflow coverage, not proof probability or conjecture completion.", proof: "The workbench now requires signed arithmetic cross-block cancellation for RH, a global sign theorem for Collatz local lifting defects, uniform signed Fourier slack in mesoscopic prime windows, and a parity-sensitive transfer from prime-weighted CRT orthogonality to positive twin principal mass." },
      ],
    },
    latest_changes: [
      { label: "Evidence-first Pages", impact: "Public entry points now lead with TICKET-239: cancellation, local lifting defects, signed reflected Fourier phase, and the limits of uniform CRT rank, while preserving TICKET-238 and the full ledger.", metric: "4 current cards / 239 tickets" },
    ...
    openProblems: [
      {
        ...
        route: "TICKET-239 exact: power decay with α&gt;1 and 2Cζ(α)&lt;1 is sufficient, while nonsummable positive Gram families refute absolute-sum necessity. / 거듭제곱 감소는 충분조건이지만 절대 행합 가능성은 필요조건이 아닙니다.",
        artifact: "next: ArithmeticWeilCrossBlockCotlarSteinCancellationBoundOnCofinalLogarithmicShells",
      },
      {
        name: "Collatz Conjecture / 콜라츠 추측",
        href: "open-problems/collatz.html",
        status: "open_not_proven / 미해결",
        route: "TICKET-239 exact: each odd prime has a fixed local lifting defect that enables or disables every common-period multiple. / 각 홀수 소수의 고정 lifting 결함이 모든 공통 주기 배수의 valuation 증인 여부를 결정합니다.",
        artifact: "next: RunBlockLocalLiftingDefectNonpositiveForEveryOddPrime",
      },
      {
        name: "Strong Goldbach / 강한 골드바흐 추측",
        href: "open-problems/goldbach.html",
        status: "open_not_proven / 미해결",
        route: "TICKET-239 exact: reflected counts are signed Fourier coefficients, and cardinality plus Parseval energy cannot force positivity. / 반사 표현수는 부호 있는 푸리에 계수이며 원소 수와 Parseval 에너지만으로 양성을 강제할 수 없습니다.",
        artifact: "next: MesoscopicPrimeWindowSignedFourierRemainderExceedsNegativeDCWithUniformSlack",
      },
      {
        name: "Twin Prime Conjecture / 쌍둥이 소수 추측",
        href: "open-problems/twin-prime.html",
        status: "open_not_proven / 미해결",
        route: "TICKET-239 exact: uniform CRT coordinates have identity Gram, yet every admissible class contains infinitely many composite pairs. / 균일 CRT 좌표의 Gram이 항등이어도 각 허용 잉여류에는 합성수 쌍이 무한히 존재합니다.",
        artifact: "next: ParitySensitiveTransferFromPrimeWeightedCRTOrthogonalityToPositiveTwinPrincipalMass",
      },
    ],    nextSteps: [
      ...openGaps.map((gap) => ({
