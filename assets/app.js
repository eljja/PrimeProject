  ...
  change_dashboard: {
    ...
    visual_rollup: {
      ...
      evidence_spine: [
        ...
        { layer: "Open-Proof", score: 99, status: "open", artifacts: ["data/open-problem/ticket251-interior-crt-cyclotomic-righteven.json", "docs/interior-crt-cyclotomic-righteven.md", "docs/interior-crt-cyclotomic-righteven.ko.md", "docs/proof-or-counterexample-program.md"], gate: "TICKET-251 proves three exact route no-go results and one elementary partial theorem, records four acyclic proof DAGs with zero machine failures, checks 11 RH concentration rows, four Collatz CRT cases, 32 Goldbach cyclotomic-unit cases, and seven twin scales with 124 witnesses while keeping every resolution counter at zero; score 99 measures workflow coverage, not proof probability or conjecture completion.", proof: "The current workbench requires a nonlocal arithmetic Weil kernel excluding concentration, canonical Fermat-quotient distribution beyond finite CRT interpolation, an arithmetic exclusion of cyclotomic-unit concentration for actual prime counts, and an all-X exclusion of x^2-2=y^k for odd k>=3." },
      ],
    },
    latest_changes: [
      { label: "Evidence-first Pages", impact: "Public entry points now lead with TICKET-251: interior concentration, finite-prime CRT interpolation, cyclotomic-unit concentration, and a right-even modulo-eight constraint, while preserving TICKET-250 and the full ledger.", metric: "4 current cards / 251 tickets" },
    ...
    openProblems: [
      {
        ...
        route: "TICKET-251 exact no-go: every continuous nonnegative local multiplier with an interior zero admits concentration escape. / 내부 영점 local multiplier는 모두 집중 도주를 허용합니다.",
        artifact: "next: NonlocalArithmeticWeilKernelExcludesInteriorConcentration",
      },
      {
        name: "Collatz Conjecture / 콜라츠 추측",
        href: "open-problems/collatz.html",
        status: "open_not_proven / 미해결",
        route: "TICKET-251 exact no-go: arbitrary targets over every finite prime set are CRT-interpolable. / 임의의 유한 소수 target은 CRT로 보간됩니다.",
        artifact: "next: CanonicalRepresentativeFermatQuotientDistributionBeyondFiniteCRTInterpolation",
      },
      {
        name: "Strong Goldbach / 강한 골드바흐 추측",
        href: "open-problems/goldbach.html",
        status: "open_not_proven / 미해결",
        route: "TICKET-251 exact no-go: cyclotomic-unit integer vectors have full support but concentrate energy on two frequencies. / cyclotomic-unit 정수 vector는 full support이면서 두 주파수로 집중합니다.",
        artifact: "next: ActualPrimeCountResidueVectorsExcludeCyclotomicUnitConcentration",
      },
      {
        name: "Twin Prime Conjecture / 쌍둥이 소수 추측",
        href: "open-problems/twin-prime.html",
        status: "open_not_proven / 미해결",
        route: "TICKET-251 partial: every right-even active pair has odd k and p=7 modulo 8; this does not force k=1. / right-even 해는 k가 홀수이고 p=7 mod 8이지만 k=1은 강제되지 않습니다.",
        artifact: "next: NoPositivePrimePowerSolutionsOfXSquareMinusTwoEqualsYOddPower",
      },
    ],    nextSteps: [
      ...openGaps.map((gap) => ({
