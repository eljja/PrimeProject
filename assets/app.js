...
const bundledProjectEvolution = {
  ...
  metrics: {
    ...
    claim_language_scanned_files: 260,
    claim_language_scanned_lines: 89081,
  ...
  change_dashboard: {
    ...
    visual_rollup: {
      ...
      evidence_spine: [
        ...
        { layer: "Open-Proof", score: 99, status: "open", artifacts: ["data/open-problem/ticket243-bandlimit-principal-unit-half-arc-dyadic-mimicry.json", "docs/bandlimit-principal-unit-half-arc-dyadic-mimicry.md", "docs/bandlimit-principal-unit-half-arc-dyadic-mimicry.ko.md", "docs/proof-or-counterexample-program.md"], gate: "TICKET-243 proves three exact route no-go theorems and one partial theorem, records four proof DAGs with zero machine failures, replays 5,130 local models through q=50,000, and keeps all four parent-conjecture resolution counters at zero; score 99 measures workflow coverage, not proof probability or conjecture completion.", proof: "The workbench now requires joint physical-frequency tightness plus a uniform signed Weil tail, fixed-base rational-Wieferich exclusion, complete small-denominator major arcs plus signed residual saving, and scale-local parity-sensitive Type II cancellation for shift two." },
      ],
    },
    latest_changes: [
      { label: "Evidence-first Pages", impact: "Public entry points now lead with TICKET-243: bandlimit noncompactness, principal-unit countermodels, half-frequency energy, and dyadic fixed-period mimicry, while preserving TICKET-242 and the full ledger.", metric: "4 current cards / 243 tickets" },
    ...
    openProblems: [
      {
        ...
        route: "TICKET-243 exact no-go: fixed Fourier support contains a normalized real-even orthonormal family, so frequency tightness alone is not compactness. / 고정 Fourier support에도 normalized real-even orthonormal family가 있어 frequency tightness만으로 compactness가 나오지 않습니다.",
        artifact: "next: JointPhysicalFrequencyTightnessAndUniformSignedGuinandWeilTailWithPositiveMargin",
      },
      {
        name: "Collatz Conjecture / 콜라츠 추측",
        href: "open-problems/collatz.html",
        status: "open_not_proven / 미해결",
        route: "TICKET-243 exact no-go: explicit Teichmüller/principal-unit countermodels break universal local transfer at order (q−1)/2 for every q>5. / 명시적 Teichmüller/principal-unit 모델이 모든 q>5에서 universal local transfer를 깨뜨립니다.",
        artifact: "next: FixedBaseRationalWieferichExclusionFor32Over27OnAllPrimeOrderCores",
      },
      {
        name: "Strong Goldbach / 강한 골드바흐 추측",
        href: "open-problems/goldbach.html",
        status: "open_not_proven / 미해결",
        route: "TICKET-243 exact no-go: the half-frequency arc has natural binary energy, so omitting it from the major arcs defeats an absolute little-o budget. / half-frequency arc는 자연 이항 에너지를 가지므로 major arc에서 누락하면 절대 little-o budget이 불가능합니다.",
        artifact: "next: CompleteSmallDenominatorMajorArcCoverageAndSignedResidualBinaryCoefficientSaving",
      },
      {
        name: "Twin Prime Conjecture / 쌍둥이 소수 추측",
        href: "open-problems/twin-prime.html",
        status: "open_not_proven / 미해결",
        route: "TICKET-243 partial: every fixed admissible periodic fingerprint has a prime/composite-successor mimic in every sufficiently large dyadic block. / 각 고정 admissible periodic fingerprint는 충분히 큰 모든 dyadic block에 소수·합성 후속항 모방자를 가집니다.",
        artifact: "next: ScaleLocalGrowingModulusTypeIICancellationForShiftTwoLambdaWithPositivePrimeMass",
      },
    ],    nextSteps: [
      ...openGaps.map((gap) => ({
