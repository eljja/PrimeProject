  ...
  change_dashboard: {
    ...
    visual_rollup: {
      ...
      release_trail: [
        ...
        { marker: "193", title: "Weighted variation, moving-modulus angles, a q=3 prime race, and variable-denominator congruences sharpen four gates", state: "bounded", measure: "RH 16 weighted-variation rows / Collatz 64 phase envelopes plus 15 fixed-modulus checks / Goldbach 3 q=3 prefixes through 28,697,817 primes with 2 algorithms / Twin 256 convergents and both signs through a 121-digit denominator / 1 exact no-go plus 3 partial theorems / 0 conjecture resolutions", proof: "TICKET-260 proves a summable weighted-downward-variation criterion, constructs fixed-modulus-equidistributed phases with linear alignment, reduces the q=3 compatible Goldbach family to a special prime race, and derives a necessary modulo-v-squared Twin sieve while all parent conjectures remain open" },
      ],
      evidence_flow: [
      ...
      evidence_spine: [
        ...
        { layer: "Open-Proof", score: 99, status: "open", artifacts: ["data/open-problem/ticket260-weighted-equidistribution-primerace-variablemod.json", "docs/weighted-equidistribution-primerace-variablemod.md", "docs/weighted-equidistribution-primerace-variablemod.ko.md", "docs/proof-or-counterexample-program.md"], gate: "TICKET-260 proves one exact no-go and three partial theorems, records four acyclic proof DAGs, checks 16 RH rows, 64 Collatz phases plus 15 fixed moduli, three Goldbach prefixes with two exact algorithms, and 256 Twin convergents under a modulo-v-squared sieve while every resolution counter stays zero; score 99 measures workflow coverage, not proof probability.", proof: "The current workbench requires actual-Weil weighted variation summability, vanishing canonical Fermat-quotient angular discrepancy, nonvanishing of a mod-3 prime race at special indices, and exclusion of the second-order denominator congruence on every Twin root convergent." },
      ],
    },
    latest_changes: [
      { label: "Evidence-first Pages", impact: "Public entry points now lead with TICKET-260: weighted variation, moving-modulus phase alignment, the q=3 Goldbach prime race, and the Twin modulo-v-squared sieve, while preserving TICKET-259 and the full ledger.", metric: "4 current cards / 260 tickets" },
    ...
    openProblems: [
      {
        ...
        route: "TICKET-260 partial theorem: summable scaled downward variation forces eventual lag positivity; actual Weil summability remains open. / 합 가능한 가중 하강변동은 eventual lag 양성을 강제하지만 실제 Weil 합 가능성은 열려 있습니다.",
        artifact: "next: ActualWeilPacketScaledDownwardVariationIsSummable",
      },
      {
        name: "Collatz Conjecture / 콜라츠 추측",
        href: "open-problems/collatz.html",
        status: "open_not_proven / 미해결",
        route: "TICKET-260 exact no-go: exponents balanced modulo every fixed integer can still align on growing prime moduli. / 모든 고정 모듈러스에서 균등한 지수도 성장하는 소수 모듈러스에서 정렬될 수 있습니다.",
        artifact: "next: CanonicalFermatQuotientAngularDiscrepancyTendsToZero",
      },
      {
        name: "Strong Goldbach / 강한 골드바흐 추측",
        href: "open-problems/goldbach.html",
        status: "open_not_proven / 미해결",
        route: "TICKET-260 partial theorem: the q=3 compatible family is exactly a special mod-3 prime-race nonvanishing problem. / q=3 호환 무한족을 특수 mod-3 소수경주 비소멸 문제로 정확히 환원했습니다.",
        artifact: "next: Q3SpecialPrimeRaceNeverTiesAtSixTimesPowerOfThreePlusThree",
      },
      {
        name: "Twin Prime Conjecture / 쌍둥이 소수 추측",
        href: "open-problems/twin-prime.html",
        status: "open_not_proven / 미해결",
        route: "TICKET-260 partial theorem: unit coefficients force a modulo-v-squared condition, and the first 256 convergents fail it. / unit coefficient는 mod v² 조건을 강제하며 처음 256개 수렴분수는 이를 통과하지 못합니다.",
        artifact: "next: NoUniqueRootConvergentSatisfiesSecondOrderDenominatorCongruence",
      },
    ],    nextSteps: [
      ...openGaps.map((gap) => ({
