...
const bundledProjectEvolution = {
  ...
  metrics: {
    ...
    claim_language_scanned_files: 250,
    claim_language_scanned_lines: 85736,
  ...
  change_dashboard: {
    ...
    visual_rollup: {
      ...
      evidence_spine: [
        ...
        { layer: "Open-Proof", score: 99, status: "open", artifacts: ["data/open-problem/ticket238-multishell-valuation-buffer-effectiverank.json", "docs/multishell-valuation-buffer-effectiverank.md", "docs/multishell-valuation-buffer-effectiverank.ko.md", "docs/proof-or-counterexample-program.md"], gate: "TICKET-238 proves four exact partial or no-go theorems, records four proof DAGs with zero machine failures, and keeps all four parent-conjecture resolution counters at zero; score 99 measures workflow coverage, not proof probability or conjecture completion.", proof: "The workbench now requires an arithmetic Weil multishell row-sum estimate, finite-palette escape for run-block valuation witnesses, mesoscopic buffered Goldbach cross-phase gain, and prime-weighted CRT Gram effective-rank divergence before the separate parity and principal-mass gaps." },
      ],
    },
    latest_changes: [
      { label: "Evidence-first Pages", impact: "Public entry points now lead with TICKET-238: multishell accumulation, valuation quantifiers, mesoscopic buffers, and effective rank, while preserving TICKET-237 and the full ledger.", metric: "4 current cards / 238 tickets" },
    ...
    openProblems: [
      {
        ...
        route: "Exact: a normalized multishell cross-row sum below one implies global positivity. No-go: pairwise positive angles alone do not imply a strict global lower bound. / 다중 shell 교차 행합 조건은 충분하지만 쌍별 양의 각도만으로는 전역 하한을 얻을 수 없습니다.",
        artifact: "next: ArithmeticWeilInnovationNormalizedCrossRowSumBelowOneOnCofinalDisjointLogarithmicShells",
      },
      {
        name: "Collatz Conjecture / 콜라츠 추측",
        href: "open-problems/collatz.html",
        status: "open_not_proven / 미해결",
        route: "Exact: adaptive valuation gaps are equivalent to affine nondivisibility, with witnesses for every run block. / 적응형 valuation gap은 affine 비가분성과 동치이며 모든 run block에 증인이 있습니다.",
        artifact: "next: RunBlockValuationWitnessEscapesEveryFixedFinitePrimePalette",
      },
      {
        name: "Strong Goldbach / 강한 골드바흐 추측",
        href: "open-problems/goldbach.html",
        status: "open_not_proven / 미해결",
        route: "Exact: an inverse-log reflected margin requires an endpoint buffer of at least order X/(log X)^2. / 역로그 반사 여유에는 최소 X/(log X)^2 차수의 끝점 buffer가 필요합니다.",
        artifact: "next: MesoscopicBufferedDyadicReflectedPrimeCrossPhaseGainWithIndependentMinorSlack",
      },
      {
        name: "Twin Prime Conjecture / 쌍둥이 소수 추측",
        href: "open-problems/twin-prime.html",
        status: "open_not_proven / 미해결",
        route: "Exact: degree-two CRT energy decay is equivalent to normalized Gram effective-rank divergence. No-go: support growth alone is insufficient. / 2차 CRT 에너지 감쇠는 Gram 유효랭크 발산과 동치이며 지지집합 증가만으로는 부족합니다.",
        artifact: "next: PrimeWeightedDegreeTwoCRTGramEffectiveRankDivergesWithUniformDiagonalControl",
      },
    ],    nextSteps: [
      ...openGaps.map((gap) => ({
