  ...
  change_dashboard: {
    ...
    visual_rollup: {
      ...
      evidence_spine: [
        ...
        { layer: "Open-Proof", score: 99, status: "open", artifacts: ["data/open-problem/ticket264-asymmetric-threshold-fixed2adic-head.json", "docs/asymmetric-threshold-fixed2adic-head.md", "docs/asymmetric-threshold-fixed2adic-head.ko.md", "docs/proof-or-counterexample-program.md"], gate: "TICKET-264 proves three partial theorems and one exact no-go, records four acyclic proof DAGs, checks 192 RH rows, 252 Collatz threshold cases, 16 Goldbach phase periods plus 242 countermodels, and the complete 38-convergent Twin subthreshold head while every resolution counter stays zero; score 99 measures workflow coverage, not proof probability.", proof: "The current workbench requires an actual-Weil one-sided envelope-sum bound, divergence of the canonical Fermat-quotient threshold cutoff, an all-level q=3 prime-race gap, and exclusion of joint ninth-order congruences on every later unique-root convergent." },
      ],
    },
    latest_changes: [
      { label: "Evidence-first Pages", impact: "Public entry points now lead with TICKET-264: asymmetric envelope, explicit threshold cutoff, fixed two-adic no-go, and Twin finite-head closure, while preserving TICKET-263 and the full ledger.", metric: "4 current cards / 264 tickets" },
    ...
    openProblems: [
      {
        ...
        route: "TICKET-264 partial theorem: one-sided envelopes give liminf packet lag at least L-A_+-A_-, and both coefficients are sharp. / 비대칭 envelope 합은 lag 하한 L-A_+-A_-를 주며 두 계수는 최적입니다.",
      ...
      },
      {
        ...
        status: "open_not_proven / 미해결",
        route: "TICKET-264 partial theorem: all fixed-h Weyl limits are equivalent to divergence of an explicit finite-data threshold cutoff. / 모든 고정 조화파 극한은 명시적 finite-data threshold cutoff의 발산과 동치입니다.",
        artifact: "next: CanonicalFermatQuotientThresholdCutoffDiverges",
      },
      {
        name: "Strong Goldbach / 강한 골드바흐 추측",
        href: "open-problems/goldbach.html",
        status: "open_not_proven / 미해결",
        route: "TICKET-264 exact no-go: every fixed two-adic q=3 tie signature plus total admits shifted non-tie count models. / 모든 고정 2진 tie signature와 총합은 이동된 non-tie 반모형을 허용합니다.",
        artifact: "next: Q3SpecialPrimeRaceAbsoluteGapAtLeastTwo",
      },
      {
        name: "Twin Prime Conjecture / 쌍둥이 소수 추측",
        href: "open-problems/twin-prime.html",
        status: "open_not_proven / 미해결",
        route: "TICKET-264 partial theorem: all 38 unique-root convergents below the explicit ninth-order threshold are unit-free. / 명시적 9차 문턱 이하 unique-root convergent 38개 전체가 unit-free입니다.",
        artifact: "next: NoLaterUniqueRootConvergentSatisfiesJointNinthOrderCongruences",
      },
    ],    nextSteps: [
      ...openGaps.map((gap) => ({
