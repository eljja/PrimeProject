  ...
  change_dashboard: {
    ...
    visual_rollup: {
      ...
      evidence_spine: [
        ...
        { layer: "Open-Proof", score: 99, status: "open", artifacts: ["data/open-problem/ticket240-route-corrections-wieferich-prime-crt.json", "docs/route-corrections-wieferich-prime-crt.md", "docs/route-corrections-wieferich-prime-crt.ko.md", "docs/proof-or-counterexample-program.md"], gate: "TICKET-240 proves four exact route-correction theorems, records four proof DAGs with zero machine failures, and keeps all four parent-conjecture resolution counters at zero; score 99 measures workflow coverage, not proof probability or conjecture completion.", proof: "The workbench now requires a signed arithmetic Weil block-symbol lower bound, universal rational-Wieferich depth domination, an explicit binary-prime major-arc margin of at least one, and a two-sided parity-breaking Lambda-Lambda main term dominating growing CRT error." },
      ],
    },
    latest_changes: [
      { label: "Evidence-first Pages", impact: "Public entry points now lead with TICKET-240: Cotlar necessity no-go, rational Wieferich reduction, Goldbach integrality equivalence, and one-sided prime CRT no-go, while preserving TICKET-239 and the full ledger.", metric: "4 current cards / 240 tickets" },
    ...
    openProblems: [
      {
        ...
        route: "TICKET-240 exact: a uniform Gram lower bound can coexist with divergent Cotlar square-root overlap sums. / 균일 Gram 하한과 발산하는 Cotlar 제곱근 겹침합은 양립하므로 절대 Cotlar 합가능성은 필요조건이 아닙니다.",
        artifact: "next: ArithmeticWeilSignedBlockOperatorSymbolHasUniformPositiveLowerBoundAfterCommonModeRemoval",
      },
      {
        name: "Collatz Conjecture / 콜라츠 추측",
        href: "open-problems/collatz.html",
        status: "open_not_proven / 미해결",
        route: "TICKET-240 exact: the local defect is a rational Wieferich-depth comparison; 1,270,605 odd primes through 20M have no positive defect. / 국소 결함을 유리 Wieferich 깊이 비교로 환원했고 2천만까지 양의 결함은 없었습니다.",
        artifact: "next: RationalWieferichDepthDominationFor32Over27Versus2Over3AtEveryOddPrime",
      },
      {
        name: "Strong Goldbach / 강한 골드바흐 추측",
        href: "open-problems/goldbach.html",
        status: "open_not_proven / 미해결",
        route: "TICKET-240 exact: signed slack above negative DC is exactly equivalent to a positive integer representation count. / 음의 DC를 넘는 부호 여유는 정수 표현수의 양성과 정확히 동치이므로 중간 목표가 아닙니다.",
        artifact: "next: BinaryPrimeMajorArcMainTermMinusAllExplicitErrorsIsAtLeastOneForEverySufficientlyLargeEvenTarget",
      },
      {
        name: "Twin Prime Conjecture / 쌍둥이 소수 추측",
        href: "open-problems/twin-prime.html",
        status: "open_not_proven / 미해결",
        route: "TICKET-240 exact: every finite one-sided CRT pattern contains infinitely many primes p with composite p+2. / 모든 유한 단측 CRT 패턴에는 p는 소수지만 p+2는 합성수인 경우가 무한히 존재합니다.",
        artifact: "next: ParityBreakingTwoSidedLambdaLambdaMainTermDominatesGrowingCRTErrorOnCofinalDyadicBlocks",
      },
    ],    nextSteps: [
      ...openGaps.map((gap) => ({
