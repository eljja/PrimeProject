...
const bundledProjectEvolution = {
  ...
  metrics: {
    ...
    claim_language_scanned_files: 252,
    claim_language_scanned_lines: 86297,
    claim_language_triggered_mentions: 131,
    claim_language_guarded_mentions: 131,
  ...
  change_dashboard: {
    ...
    visual_rollup: {
      ...
      evidence_spine: [
        ...
        { layer: "Open-Proof", score: 99, status: "open", artifacts: ["data/open-problem/ticket241-finite-information-canonical-errors.json", "docs/finite-information-canonical-errors.md", "docs/finite-information-canonical-errors.ko.md", "docs/proof-or-counterexample-program.md"], gate: "TICKET-241 proves four exact information-boundary theorems, records four proof DAGs with zero machine failures, extends one bounded fixed-base scan to 100M, and keeps all four parent-conjecture resolution counters at zero; score 99 measures workflow coverage, not proof probability or conjecture completion.", proof: "The workbench now requires convergence of signed Guinand-Weil finite sections without an artificial diagonal, fixed-base Fermat-quotient line avoidance, a fixed binary-prime arc decomposition with a uniform positive targetwise certificate, and a growing-modulus parity-sensitive Type II bound for shift two." },
      ],
    },
    latest_changes: [
      { label: "Evidence-first Pages", impact: "Public entry points now lead with TICKET-241: finite prime-cosine rank, principal-unit no-go plus a 100M fixed-base scan, canonical Goldbach error contracts, and finite-periodic Twin mimicry, while preserving TICKET-240 and the full ledger.", metric: "4 current cards / 241 tickets" },
    ...
    openProblems: [
      {
        ...
        route: "TICKET-241 exact: finite unsigned prime-cosine kernels are PSD with rank at most twice their prime support, so an artificial diagonal floor is RH-blind. / 유한 무부호 소수 코사인 핵은 소수 지지 크기의 두 배 이하 계수를 가지므로 인공 대각 하한은 RH와 무관합니다.",
        artifact: "next: SignedGuinandWeilFiniteSectionsConvergeWithoutArtificialDiagonalForEveryAdmissibleTestFamily",
      },
      {
        name: "Collatz Conjecture / 콜라츠 추측",
        href: "open-problems/collatz.html",
        status: "open_not_proven / 미해결",
        route: "TICKET-241 exact: principal-unit algebra admits the bad Fermat line, while all 5,761,453 primes through 100M avoid it for the actual bases. / 주단위원 대수에는 나쁜 Fermat 직선이 존재하지만 실제 밑수는 1억 이하 소수 5,761,453개에서 이를 피했습니다.",
        artifact: "next: FixedBaseFermatQuotientLineAvoidanceFor5Fq2Equals3Fq3UnlessFq2EqualsFq3",
      },
      {
        name: "Strong Goldbach / 강한 골드바흐 추측",
        href: "open-problems/goldbach.html",
        status: "open_not_proven / 미해결",
        route: "TICKET-241 exact: signed errors restate representation, while absolute-error success depends on a fixed decomposition and norm. / 부호 오차는 표현 존재를 다시 쓰는 것이고 절대오차 성공은 고정된 분해와 노름에 의존합니다.",
        artifact: "next: FixedBinaryPrimeArcDecompositionHasUniformTargetwisePositiveLowerCertificate",
      },
      {
        name: "Twin Prime Conjecture / 쌍둥이 소수 추측",
        href: "open-problems/twin-prime.html",
        status: "open_not_proven / 미해결",
        route: "TICKET-241 exact: every finite periodic fingerprint has infinitely many primes p preserving the fingerprint with composite p+2. / 모든 유한 주기 fingerprint에는 같은 특징을 보존하면서 p+2가 합성수인 소수 p가 무한히 존재합니다.",
        artifact: "next: GrowingModulusParitySensitiveTypeIIBoundForShiftTwoLambdaOnInfinitelyManyDyadicBlocks",
      },
    ],    nextSteps: [
      ...openGaps.map((gap) => ({
