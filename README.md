...
The Research Atlas and Project Evolution panels read `data/project_evolution.json` and now present a condensed research narrative: the supported contribution map, evidence ladder, open-problem proof map, next academic blockers, six decisive metrics, a seventy-nine-step visual change trail, a Hardening Map for recent collection-contract fixes, one Evidence Spine for artifact-backed readiness, and a claim-boundary view that separates supported controlled-synthetic results from blocked real-world and Bitcoin attribution claims. Its publication layer includes the post-pack consistency audit so the visible research history does not stop one governance step before the Evidence Pack panel.
...
The latest proof-search track is TICKET-119. Commit `87bdcf9` publicly froze the unseen 16M horizon, the unchanged denominator-independent adjacent-shell rule, a one-run reporting policy, and strict positivity of the same finite lower expression before the result existed. The registered endpoint passes with canonical numerator budget `5.728M`, adverse-to-known ratio `0.802678`, normalized margin `19.7322%`, and finite lower expression `+1,479,021.8`. The 8M margin was `4.2047%`; this improvement is a finite secondary observation, not monotonicity or a scaling theorem. The next target remains `EventuallySubcriticalCanonicalAdjacentDyadicPairVaughanEndpointBudget`, with `UniformLowDivisorCanonicalPairDispersion` as the first sublemma. All four conjectures remain open. See [TICKET119](docs/twin-canonical-pair-doubling-holdout.md), [TICKET118](docs/twin-canonical-adjacent-pair-holdout.md), and [TICKET117](docs/twin-dyadic-mobius-endpoint-gram-audit.md).

한국어 최신 요약: 최신 탐색은 TICKET-119입니다. 16M 결과를 계산하기 전에 커밋 `87bdcf9`로 기존 인접 dyadic shell pairing, 모든 Farey 분모에 동일한 partition, 단 한 번의 실행과 결과 무조건 보고, 같은 양수 하한 판정식을 공개 고정했습니다. 최초 실행은 canonical 예산 `5.728M`, adverse/known 비율 `0.802678`, 정규화 margin `19.7322%`, 유한 하한 `+1,479,021.8`로 통과했습니다. 8M의 margin `4.2047%`보다 커졌지만 두 유한 행은 단조성이나 점근 정리가 아닙니다. 다음 목표는 `EventuallySubcriticalCanonicalAdjacentDyadicPairVaughanEndpointBudget`이고 첫 하위 정리는 `UniformLowDivisorCanonicalPairDispersion`입니다. 네 난제는 모두 여전히 미해결입니다.
