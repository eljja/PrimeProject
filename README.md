...
- Publishes a machine-readable collection submission contract with task templates, required record fields, checksum rules, provenance requirements, feature-vector schema rules, and forbidden public fields.
...
The Project Evolution panel reads `data/project_evolution.json` and visualizes how the work moved from prime-measure experiments to controlled attribution, real-world baseline scaffolding, readiness gates, evidence packs, and a maturity dashboard that summarizes the latest research changes. It now includes a visual change trail, evidence-flow strip, and Research Delta view that contrast the original exploratory demo with the current gated scaffold, separate scale/signal/reality/publication progress, show the submission-contract milestone, and show which claim lanes are allowed or blocked.
...
The Baseline Lab panel reads `data/baselines/real_world/manifest.json`, `data/collection_matrix.json`, `data/collection_power.json`, `data/provenance_requirements.json`, `data/provenance_audit.json`, `data/baseline_acceptance.json`, `data/baseline_promotion_plan.json`, `data/collection_handoff.json`, `data/collection_submission_contract.json`, and `data/collection_intake.json` to show which real-world baseline families are registered, whether targets are only coarse screening or strong enough for tighter claims, which baselines still block publication, which public-safe collection tasks should be executed first, what collectors must submit, and whether submitted aggregate artifacts are acceptable without feature-vector contract failures, duplicate submission collisions, or reused aggregate checksums.
