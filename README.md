...
The latest proof-search track is TICKET-120. It decomposes the first canonical low-divisor pair saving into scalar mean and centered Hilbert-angle terms on 8 scales and 248 denominator rows. At 16M the pair saves `19.7226%` against singleton treatment, but scalar mean cancellation explains only `0.0069%` of that saving; the finite effect is almost entirely centered geometry. TICKET-120 proves the elementary nonnegative pair-saving identity and exactly refutes `UniversalFixedFractionLowDivisorPairSavingUnderGramContract` with an aligned rank-one PSD witness whose saving is zero. Therefore Gram positivity alone is discarded as a source of uniform power saving. The next arithmetic target is `VaughanLowDivisorDenominatorSummedAngleGap`; all four conjectures remain open. See [TICKET120](docs/twin-low-divisor-pair-savings-audit.md), [TICKET119](docs/twin-canonical-pair-doubling-holdout.md), and [TICKET118](docs/twin-canonical-adjacent-pair-holdout.md).

한국어 최신 요약: 최신 탐색은 TICKET-120입니다. 첫 canonical low-divisor pair의 절감을 8개 규모와 248개 분모에서 scalar mean과 centered Hilbert-angle 항으로 분해했습니다. 16M의 singleton 대비 절감은 `19.7226%`지만 scalar mean 상쇄가 설명하는 비율은 `0.0069%`뿐이어서 실제 유한 효과는 거의 전부 centered geometry에서 나옵니다. 두 블록 pairing이 singleton보다 나쁘지 않다는 초등 항등식은 성립하지만, 정렬된 rank-one PSD 반례에서는 절감이 정확히 0이므로 약한 Gram 가정만으로 고정 절감률을 얻는 보조정리는 거짓입니다. 다음 산술 목표는 `VaughanLowDivisorDenominatorSummedAngleGap`이며 네 난제는 모두 미해결입니다.
