...
The Research Atlas and Project Evolution panels read `data/project_evolution.json` and now present a condensed research narrative: the supported contribution map, evidence ladder, open-problem proof map, next academic blockers, six decisive metrics, an eighty-eight-step visual change trail, a Hardening Map for recent collection-contract fixes, one Evidence Spine for artifact-backed readiness, and a claim-boundary view that separates supported controlled-synthetic results from blocked real-world and Bitcoin attribution claims. Its publication layer includes the post-pack consistency audit so the visible research history does not stop one governance step before the Evidence Pack panel.
...
The latest proof-search track is TICKET-128. For RH it proves that compact support reduces the arithmetic side of a Weil explicit formula to the finite list `p^m<=B`; at `B=1,000,000` this is 78,734 terms, while admissible-core density and archimedean intervals remain open. For Collatz it corrects a second interpretation: TICKET-127's 4,027,109 objects were unresolved **lift cylinders**, not surviving integer counterexamples. Direct replay closes every nontrivial representative below `2^28`; the maximum accelerated descent time is 249 at `217,740,015`, but no unbounded-prefix theorem follows. For Goldbach an exact rational tail certificate gives `2C2>1.31917`, so a pointwise residual theorem with the relaxed constant `K<=55` would suffice above `4e18`; that residual theorem is not proved. For Twin Prime an explicit sequence refutes endpoint-only interpolation, while the exact repair theorem requires a within-block envelope satisfying `0.92c+delta<1`. Parity and exact-gap positivity remain open. All four conjectures remain open. See [TICKET128](docs/finite-core-prefix-constant-interpolation.md), [TICKET127](docs/exception-repair-and-effective-bridges.md), and [TICKET126](docs/route-correction-and-premise-closure.md).

한국어 최신 요약: 최신 탐색은 TICKET-128입니다. RH에서는 compact support가 Weil 산술항을 `p^m<=B`인 유한 합으로 바꾼다는 점을 증명했지만, 허용 핵심집합의 조밀성과 archimedean 구간 계산은 남아 있습니다. Collatz에서는 앞서 말한 4,027,109개가 실제 반례 후보가 아니라 원통 전체 lift를 한 번에 닫지 못한 대표였음을 교정했습니다. 이 대표들은 모두 직접 하강했고 `n<2^28`에서 생존자는 0이지만, 모든 정수를 덮는 균일 정리는 아닙니다. Goldbach에서는 정확한 유리수 꼬리 하한으로 `A>1.31917`을 얻어 필요한 점별 잔차 목표를 `K<=55`로 완화했습니다. Twin은 dyadic endpoint-only 추론의 명시적 반례를 만들고 `0.92c+delta<1`인 전 구간 포락선이 필요함을 증명했습니다. 네 난제는 모두 미해결입니다.
...
최신 난제 연구 트랙은 TICKET-128입니다. [유한 핵심·prefix 폐쇄·상수 강화·보간 보고서](docs/finite-core-prefix-constant-interpolation.md)는 RH 유한 소수항 환원, Collatz 원통/정수 후보 분리와 28비트 직접 폐쇄, Goldbach `A>1.31917` 및 `K=55` 충분조건, Twin endpoint-only 반례와 all-scale 보간 정리를 한글과 영문으로 정리합니다. 다음 공격 대상은 각각 archimedean 구간과 핵심집합 조밀성, 무한 prefix 폐쇄 또는 균일 순위함수, 점별 Goldbach 잔차 `K<=55`, 실제 Vaughan 구간 포락선입니다. 네 문제는 모두 미해결입니다.
