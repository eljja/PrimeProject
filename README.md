...
The latest proof-search track is TICKET-48, an automaton/reachability split. It keeps the 28-bit Collatz 16-edge positive-debt lasso, strengthens the no-go from bounded suffix memory to fixed finite total deterministic state repairs by a period-map argument, and then separately tests concrete residue-lift reachability. The bounded probe found four start-template candidates, two concrete positive steps, and no complete positive-pressure lasso period; this is not a Collatz proof or counterexample. The remaining decisive target is a symbolic reachability-exclusion theorem for the lasso family, a concrete periodic lift witness at a larger horizon, or a genuinely ordinal/unbounded measure.
...
최신 난제 연구 트랙은 TICKET-48입니다. 28-bit Collatz exact-template pressure graph의 16-edge positive-debt lasso를 유지하면서, bounded suffix-memory보다 넓은 “고정된 유한상태 total deterministic update”도 period-map 반복 때문에 strict descent가 될 수 없음을 정리했습니다. 그 뒤 이 추상 lasso가 실제 residue lift 경로로 이어지는지도 따로 검사했고, bounded probe에서는 시작 후보 4개 중 2단계 positive step까지만 이어진 뒤 3단계에서 끊겼습니다. 이 결론은 Collatz 증명이나 반례가 아닙니다. 다음 핵심 목표는 이 lasso family의 symbolic reachability-exclusion theorem, 더 큰 horizon에서의 concrete periodic lift witness, 또는 진짜 ordinal/unbounded measure입니다.
