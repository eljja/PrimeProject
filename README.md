...
The Research Atlas and Project Evolution panels read `data/project_evolution.json` and now present a condensed research narrative: the supported contribution map, evidence ladder, open-problem proof map, next academic blockers, six decisive metrics, a ninety-six-step visual change trail, a Hardening Map for recent collection-contract fixes, one Evidence Spine for artifact-backed readiness, and a claim-boundary view that separates supported controlled-synthetic results from blocked real-world and Bitcoin attribution claims. Its publication layer includes the post-pack consistency audit so the visible research history does not stop one governance step before the Evidence Pack panel.
...
The latest proof-search track is TICKET-136. For RH it turns absolute core-tail row/column sums into a Schur certificate and proves that entrywise matrix decay alone cannot control operator norm. For Collatz it derives the exact affine-correction condition `2^S n^k <= (3n+1)^k` required by every non-descending prefix of a hypothetical least counterexample. For Goldbach it proves that a fixed-wheel rough stratum has linear size, so the powers-of-two `O(log log X)` moment order cannot control that full layer. For Twin Prime it proves that every finite rational Fourier feature algebra still factors through a finite congruence transcript and inherits a CRT composite-pair lift. All four conjectures remain open and none is close to proof. See [TICKET136](docs/scale-sensitive-obstructions-and-affine-bridge.md), [TICKET135](docs/conditional-bridges-and-exceptional-set.md), and [TICKET134](docs/uniformity-thresholds-and-scale-no-go.md).
