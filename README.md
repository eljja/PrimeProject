...
The Research Atlas and Project Evolution panels read `data/project_evolution.json` and now present a condensed research narrative: the supported contribution map, evidence ladder, open-problem proof map, next academic blockers, six decisive metrics, a seventy-four-step visual change trail, a Hardening Map for recent collection-contract fixes, one Evidence Spine for artifact-backed readiness, and a claim-boundary view that separates supported controlled-synthetic results from blocked real-world and Bitcoin attribution claims. Its publication layer includes the post-pack consistency audit so the visible research history does not stop one governance step before the Evidence Pack panel.
...
The latest proof-search track is TICKET-114. It opens the 31 TICKET-113 denominator blocks by reduced numerator and proves an exact Ramanujan-mean/centered-remainder decomposition. The centered term has a sharp projected L2 support bound under the explicitly weak zero-real-mean and norm contract; an opposite projected-phase vector attains equality, so improving this bound requires arithmetic information from the actual Vaughan coefficients. At 4M the fully sign-free adverse budget is `82.50%` of the known major-plus-Type-I-minor term and leaves finite lower expression `+327,951.0`, but it fails at the first three audited scales and is positive only from 1M through 4M. This is theorem-selection evidence, not an all-scale result. The next target is `EventuallySubcriticalVaughanCenteredFareyNumeratorDispersionBudget`. See [TICKET114](docs/twin-ramanujan-numerator-dispersion-audit.md), [TICKET113](docs/twin-farey-denominator-endpoint-audit.md), and [TICKET112](docs/twin-farey-endpoint-abel-audit.md).

한국어 최신 요약: 최신 탐색은 TICKET-114입니다. 31개 Farey 분모 블록을 분자별로 다시 열어 Ramanujan 평균항과 중심화 편차항으로 정확히 분해했습니다. 중심화 항의 projected L2 경계는 현재 명시한 약한 계약 아래 최적이며, 반대 위상 벡터가 equality를 달성합니다. 4M에서는 평균항의 부호까지 모두 불리하게 처리한 예산이 알려진 양의 항의 `82.50%`이고 유한 하한 `+327,951.0`을 남기지만, 첫 세 규모에서는 실패합니다. 따라서 다음 목표는 실제 Vaughan Möbius/약수 계수로부터 충분히 큰 모든 `X`에 대한 `EventuallySubcriticalVaughanCenteredFareyNumeratorDispersionBudget`을 증명하거나, 그 조건을 만족하는 산술적 반례 계수를 구성하는 것입니다. 네 난제는 모두 여전히 미해결입니다.
