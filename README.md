...
The Research Atlas and Project Evolution panels read `data/project_evolution.json` and now present a condensed research narrative: the supported contribution map, evidence ladder, open-problem proof map, next academic blockers, six decisive metrics, a seventy-six-step visual change trail, a Hardening Map for recent collection-contract fixes, one Evidence Spine for artifact-backed readiness, and a claim-boundary view that separates supported controlled-synthetic results from blocked real-world and Bitcoin attribution claims. Its publication layer includes the post-pack consistency audit so the visible research history does not stop one governance step before the Evidence Pack panel.
...
The latest proof-search track is TICKET-117. It keeps the Möbius sign inside actual dyadic outer-divisor blocks, lifts every block through the same half-Farey endpoint map, and proves the exact real-Gram identity for their centered vectors. Singleton dyadic triangles still worsen the fully signed budget on all six scales and close none. At 4M they reduce TICKET-116's separated-sign budget from `4.187M` to `2.326M`; a globally fixed contiguous partition with at most two adjacent shells per group further reduces it to `1.786M`, leaving a finite closure shortfall of `1,236.3`. The same pairing appears at 2M and 4M, but this finite selection is not an eventual theorem. The next target is `EventuallySubcriticalAdjacentDyadicPairVaughanEndpointBudget`. All four conjectures remain open. See [TICKET117](docs/twin-dyadic-mobius-endpoint-gram-audit.md), [TICKET116](docs/twin-mobius-sign-cyclotomic-audit.md), and [TICKET115](docs/twin-complex-cyclotomic-dispersion-audit.md).

한국어 최신 요약: 최신 탐색은 TICKET-117입니다. 실제 Vaughan Type-II 바깥 약수를 dyadic block으로 나누되 각 블록 안의 Möbius 부호를 유지하고, 동일한 Farey endpoint 사상을 통과시켜 평균·중심 벡터·실수 Gram 행렬을 정확히 재구성했습니다. singleton dyadic 삼각 경계는 여섯 규모 모두 완전 signed 예산보다 나쁘지만, 4M에서 TICKET-116의 부호 분리 예산 `4.187M`을 `2.326M`으로 줄입니다. 인접 shell을 최대 두 개씩 묶는 전역 연속 partition은 이를 `1.786M`까지 낮춰 유한 폐쇄 부족분을 `1,236.3`으로 축소했습니다. 같은 pairing이 2M과 4M에서 나타나지만 유한 선택 결과일 뿐 전 규모 정리가 아닙니다. 다음 목표는 `EventuallySubcriticalAdjacentDyadicPairVaughanEndpointBudget`이며 네 난제는 모두 여전히 미해결입니다.
...
최신 난제 연구 트랙은 TICKET-76입니다. TICKET75가 찾은 압축-상태성장 trade-off를 exact boundary quotient recurrence로 설명했습니다. 고정 low-bit 수를 덧붙이는 방식은 unresolved lift마다 네 비트의 정보가 추가로 필요해 닫히지 않습니다. 다음 공격은 실제 reachable quotient에 이 정보 증가를 막는 산술 제약이 있는지 증명하거나, full 2-adic recurrence에서 all-depth pressure path를 구성하는 것입니다. 문제별 유지·폐기 근거는 [네 난제 통합 연구 정리](docs/open-problem-research-consolidation-2026-07-10.md)에 기록합니다.
