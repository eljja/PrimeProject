...
The latest proof-search track is TICKET-72, an infinite-frontier lift-closure audit after the TICKET71 coordinate tradeoff. TICKET71 showed that the full valuation-word plus residue mod `2^16` coordinate separates the bounded branch set with zero mixed transition keys, but it is too large to be treated as an infinite invariant. TICKET72 rebuilds the compact obstruction: the TICKET70 frontier still has branch weight 792,064, the compact coordinate has 22,219 mixed transition keys, and 20,752 of those carry open pressure after rank descent is separated out as progress. It then lifts the top 8 compact mixed keys. The second lift has 36,848 rows, including 6,857 open-pressure rows and 4,142 open-pressure mixed-key re-entries. A capped third probe has 32,768 rows, 12,300 open-pressure rows, and 6,448 open-pressure mixed-key re-entries. The best compact second-lift coordinate, `base_tail12_residue65536`, still leaves 540 mixed transition keys; the full-word coordinate closes the bounded rows but remains an overfit guard. This is not a Collatz proof or counterexample; the next target is `CompactMixedKeyInvariantOrPersistentLiftChain`.

한국어 최신 요약: 최신 증명 탐색은 TICKET-72입니다. TICKET71은 full valuation-word와 residue mod `2^16`을 붙이면 제한된 branch 집합에서는 mixed transition key가 0개가 되지만, 그 좌표가 너무 커서 무한 불변량이라고 부를 수 없다는 한계를 분리했습니다. TICKET72는 compact 좌표에서 남은 장애물을 다시 구성했습니다. TICKET70 frontier의 branch weight는 여전히 792,064이고, compact 좌표에는 mixed transition key 22,219개가 남습니다. 이 중 20,752개는 rank descent를 "압력"이 아니라 "진전"으로 분리한 뒤에도 열린 pressure를 갖습니다. 상위 8개 mixed key를 한 단계 더 올리면 second lift는 36,848개 row를 만들고, 그 안에 open-pressure row 6,857개와 open-pressure mixed-key 재진입 4,142개가 남습니다. 제한된 third probe에서도 32,768개 row 중 open-pressure row 12,300개와 open-pressure mixed-key 재진입 6,448개가 남았습니다. 가장 좋은 compact 후보 `base_tail12_residue65536`도 mixed transition key 540개를 남기며, full-word 좌표는 제한된 row를 닫지만 여전히 과적합 방어선으로만 취급합니다. 이것은 콜라츠 증명이나 반례가 아니라, 다음 목표를 `CompactMixedKeyInvariantOrPersistentLiftChain`으로 좁힌 결과입니다.
