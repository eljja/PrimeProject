...
The latest proof-search track is TICKET-130. For RH it proves that every TICKET-129 rational-bump core value of the fixed Weil functional is a computable real, making strict-negative counterexample search a semidecision; universal nonnegativity remains open. For Collatz it corrects and retires finite valuation-cap language extinction: an explicit mechanical word survives every finite depth and every prefix is naturally realizable, although the entire cap language has the exact exponential mass bound `(65/48)rho^j` with `rho=0.9466204159695351...`. For Goldbach it proves `K=56` is the largest integer available to the fixed `H=4*10^18`, uniform-singular-series endpoint architecture; the pointwise `K=56` residual theorem is still open. For Twin Prime it proves `D(Y)=Q_Y R(Y)` and reduces the within-block target to the dimensionless threshold `limsup R(2^j)<2/23`. Parity and exact-gap positivity remain open. All four conjectures remain open. See [TICKET130](docs/computability-cap-language-optimality.md), [TICKET129](docs/enumerable-core-valuation-cap-endpoint-budget.md), and [TICKET128](docs/finite-core-prefix-constant-interpolation.md).

한국어 최신 요약: 최신 탐색은 TICKET-130입니다. RH에서는 TICKET-129 유리 bump 핵심의 각 Weil 값이 계산 가능한 실수임을 증명하여 엄격한 음수 반례 탐색을 반결정 절차로 만들었지만 전칭 비음성은 아직 미증명입니다. Collatz에서는 유한 valuation-cap 언어가 소멸한다는 기존 목표가 불가능함을 정확한 무한 생존 word로 증명해 폐기했고, 대신 허용 언어의 원통 질량이 `rho=0.9466204159695351...`인 지수 상한으로 0에 접근함을 증명했습니다. Goldbach에서는 현재 cutoff와 균일 특이급수 계수 구조 안에서 `K=56`이 최적 정수임을 증명했지만 점별 잔차 정리는 미증명입니다. Twin Prime에서는 `D(Y)=Q_Y R(Y)`를 증명하고 실제 상대 증분 불균형 목표를 `limsup R(2^j)<2/23`으로 정밀화했습니다. 네 난제는 모두 미해결입니다.
...
최신 난제 연구 트랙은 TICKET-130입니다. [계산 가능성·cap 언어 no-go·경로 최적성 보고서](docs/computability-cap-language-optimality.md)는 RH 핵심값 계산 가능성, Collatz cap 언어의 유한 소멸 불가능성과 지수적 밀도 0, Goldbach 고정 endpoint 구조에서 `K=56`의 최적성, Twin Prime 상대 증분 임계값 `2/23`을 한글과 영문으로 정리합니다. 다음 공격 대상은 각각 모든 유리 bump 핵심값의 비음성 인증, 모든 무한 cap 경로의 자연수 실현에 대한 Archimedean 배제, 점별 Goldbach 잔차 `K<=56`, 실제 Vaughan 상대 증분 불균형의 `limsup<2/23` 정리입니다. 네 문제는 모두 미해결입니다.
