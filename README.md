...
The latest proof-search track is TICKET-118. Before evaluating 8M, commit `5b52d4d` froze a canonical rule that pairs consecutive outer-divisor dyadic shells, applies one partition to every Farey denominator, and passes only on a positive finite lower expression. The preregistered holdout passes: the canonical numerator budget is `3.413M`, or `1.193875x` the fully signed budget, and the finite lower expression is `+156,727.0`. The canonical partition also equals the post-hoc best width-two partition, but that is secondary evidence. This is one finite post-registration closure, not an eventual bound or a Twin Prime proof. The next target is `EventuallySubcriticalCanonicalAdjacentDyadicPairVaughanEndpointBudget`. All four conjectures remain open. See [TICKET118](docs/twin-canonical-adjacent-pair-holdout.md), [TICKET117](docs/twin-dyadic-mobius-endpoint-gram-audit.md), and [TICKET116](docs/twin-mobius-sign-cyclotomic-audit.md).

한국어 최신 요약: 최신 탐색은 TICKET-118입니다. 8M 계산 전에 커밋 `5b52d4d`로 인접 dyadic shell을 순서대로 두 개씩 묶고 모든 Farey 분모에 같은 partition을 적용하며 유한 하한이 양수일 때만 통과하는 규칙을 고정했습니다. 사전등록 holdout은 canonical 예산 `3.413M`, fully signed 대비 `1.193875배`, 유한 하한 `+156,727.0`으로 통과했습니다. canonical partition이 사후 최적 폭-2 partition과도 같았지만 이는 2차 관측입니다. 단 한 번의 유한 폐쇄이므로 eventual theorem이나 쌍둥이 소수 증명이 아닙니다. 다음 목표는 `EventuallySubcriticalCanonicalAdjacentDyadicPairVaughanEndpointBudget`이며 네 난제는 모두 여전히 미해결입니다.
