...
The Research Atlas and Project Evolution panels read `data/project_evolution.json` and now present a condensed research narrative: the supported contribution map, evidence ladder, open-problem proof map, next academic blockers, six decisive metrics, an eighty-three-step visual change trail, a Hardening Map for recent collection-contract fixes, one Evidence Spine for artifact-backed readiness, and a claim-boundary view that separates supported controlled-synthetic results from blocked real-world and Bitcoin attribution claims. Its publication layer includes the post-pack consistency audit so the visible research history does not stop one governance step before the Evidence Pack panel.
...
The latest proof-search track is TICKET-123. It factors the TICKET-122 full canonical margin exactly as `delta = 1-(1-eta)rho-epsilon`, where `eta=D/S` is the canonical saving fraction, `rho=S/K` is the singleton-to-known ratio, and `epsilon=E/K` is the boundary ratio. It proves `CanonicalDefectRatioClosureBridge`: uniform bounds `D>=eta*S`, `S<=rho*K`, `E<=epsilon*K`, and `(1-eta)rho+epsilon<=1-delta` imply a `delta*K` lower margin. Four explicit families refute saving-only, ratio-without-boundary, non-strict compatibility, and finite-prefix inference. Across the 8 finite scales, exact closure occurs twice and certificate closure once; all identities reproduce below `2.23e-16`. The 8M-to-16M margin gain `+0.155274` is decomposed exactly into eta `-0.038066`, rho `+0.189393`, and epsilon `+0.003947`, so the finite improvement is driven by `S/K`, not a stronger saving fraction. This is descriptive, not asymptotic. The next target is `VaughanCanonicalDefectRatioTriple`. Finite Jensen, finite Collatz stopping-time, and mean Goldbach singular-series proxies are explicitly rejected while their independent infinite targets are preserved. All four conjectures remain open. See [TICKET123](docs/canonical-defect-ratio-closure-bridge.md), [TICKET122](docs/twin-canonical-joint-defect-audit.md), and [TICKET121](docs/twin-balance-angle-defect-audit.md).

한국어 최신 요약: 최신 탐색은 TICKET-123입니다. TICKET-122의 전체 여유를 `delta = 1-(1-eta)rho-epsilon`으로 정확히 분해했습니다. `eta`는 정준 pair 절감률, `rho`는 독립 예산/알려진 양의 항 비율, `epsilon`은 경계 예산 비율입니다. 이 세 비율의 호환 조건이 양의 전체 여유를 준다는 대수 정리를 증명했고, 절감률만 보는 방법, 경계항 없는 예산비 방법, 등호만 있는 호환 조건, 유한 통과에서 무한 통과를 추론하는 방법을 각각 반례족으로 폐기했습니다. 8M에서 16M으로의 유한 개선은 절감률 증가가 아니라 `rho=S/K` 감소가 주도했습니다. 하지만 세 비율의 균일한 점근 상수는 아직 증명되지 않았습니다. RH의 유한 Jensen, Collatz의 유한 stopping-time, Goldbach의 평균 singular-series 프록시도 증명 경로에서 명시적으로 제외했습니다. 다음 목표는 `VaughanCanonicalDefectRatioTriple`이며 네 난제는 모두 미해결입니다.
...
최신 난제 연구 트랙은 TICKET-123입니다. Twin의 전체 정준 예산 여유를 pair 절감률 `eta`, 독립 예산비 `rho`, 경계 예산비 `epsilon`으로 정확히 분해하고, 세 비율의 호환 조건이 양의 여유를 준다는 대수 정리를 증명했습니다. 동시에 절감률 하나, 경계항 없는 예산비, 비엄격 호환, 유한 통과 추론을 반례족으로 폐기했습니다. 다음 공격은 실제 Vaughan 계수에 대해 호환 가능한 세 비율 상수를 충분히 큰 모든 규모에서 증명하거나, 모든 상수 후보를 깨는 Vaughan-realizable 무한 규모열을 구성하는 것입니다. RH, Collatz, Goldbach의 독립 목표는 전이 없이 유지하며 네 문제는 모두 미해결입니다. 문제별 유지·폐기 근거는 [네 난제 통합 연구 정리](docs/open-problem-research-consolidation-2026-07-10.md)에 기록합니다.
