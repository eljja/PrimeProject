...
The Research Atlas and Project Evolution panels read `data/project_evolution.json` and now present a condensed research narrative: the supported contribution map, evidence ladder, open-problem proof map, next academic blockers, six decisive metrics, a twenty-one-step visual change trail, a Hardening Map for recent collection-contract fixes, one Evidence Spine for artifact-backed readiness, and a claim-boundary view that separates supported controlled-synthetic results from blocked real-world and Bitcoin attribution claims. Its publication layer includes the post-pack consistency audit so the visible research history does not stop one governance step before the Evidence Pack panel.
...
The latest proof-search track is TICKET-61, a symbolic failure-offset audit. TICKET60 found that `low40 + failure_offset` separates the 58 mixed low40 cylinders, but `failure_offset` is replay-derived. TICKET61 forbids that circular key and tests pre-replay coordinates instead. Across the same selected population, there are 162 low40 cylinders and 535 start-template lifts; the 58 mixed cylinders contain 210 lifts. `low40` alone leaves 58 failure-offset collision groups, and `low40 + certificate_prefix_length` still leaves 36 groups / 81 ambiguous rows. The first pre-replay joint separator is `low40 + high_extension mod 16`, which determines failure offset, observed outcome, and boundary-match status on the selected data. This is real progress toward a proof route, but it remains finite selected-cylinder evidence; the missing step is a symbolic mod-16 transition theorem or automaton-counted cover. No full proof or counterexample is claimed.

한국어 최신 요약: 최신 proof-search는 TICKET-61입니다. TICKET60은 `low40 + failure_offset`이 58개 mixed low40 cylinder를 가른다는 것을 보였지만, `failure_offset`은 trajectory replay(궤도 재생)를 해본 뒤에야 알 수 있는 좌표라서 그대로는 순환적입니다. TICKET61은 이 좌표를 금지하고, replay 전에 알 수 있는 pre-replay 좌표만 검사했습니다. 같은 선택 집합에는 low40 cylinder 162개와 start-template lift 535개가 있고, mixed cylinder 안에는 lift 210개가 있습니다. `low40`만 쓰면 failure-offset collision group이 58개 남고, `low40 + certificate_prefix_length`를 써도 36개 group / 81개 ambiguous row가 남습니다. 처음으로 failure offset, 관측 outcome, boundary-match 상태를 동시에 결정적으로 가르는 pre-replay 좌표는 `low40 + high_extension mod 16`입니다. 이것은 증명 경로를 더 날카롭게 만든 진전이지만, 아직 유한한 선택 cylinder 증거입니다. 남은 핵심은 mod-16 좌표의 symbolic transition theorem 또는 automaton-counted cover입니다.
...
최신 난제 연구 트랙은 TICKET-61입니다. 기존 TICKET49의 all-phase next-valuation 장애물은 TICKET50에서 32비트 예외로 반박됐고, TICKET51/TICKET52는 32비트와 48비트 near-lasso 후보를 terminal branch에서 닫았습니다. TICKET53은 현재 추출된 phase-15 lasso family가 terminal target에 도달할 수 없다는 상징 terminal mismatch theorem으로 정리했고, TICKET54는 남은 후보를 phase-5 gate로 압축했습니다. TICKET55는 gate crosser tunnel을 닫았고, TICKET56은 exact32 lasso route 전체를 finite partition으로 닫으면서도 simple projection-closure 귀납이 틀린 경로임을 분리했습니다. TICKET57은 exact32 결정성에 `residue mod 2^28` boundary가 필요함을 보였고, TICKET58은 그 boundary가 48비트 샘플 lift에서 보존되지 않는 mismatch를 찾았습니다. TICKET59는 mismatch/match seed를 low40 cylinder로 올려 41,472개 48-bit extension을 정확히 열거했고, TICKET60은 mixed cylinder의 첫 결정적 separator가 `low40 + failure_offset`임을 보였습니다. TICKET61은 replay-derived 좌표를 버리고 `low40 + high_extension mod 16`이라는 pre-replay 좌표가 failure offset까지 결정함을 확인했습니다. 다음 목표는 이 mod-16 좌표가 lift 전이에서 닫힌다는 symbolic transition theorem입니다.
