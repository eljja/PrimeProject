...
The Research Atlas and Project Evolution panels read `data/project_evolution.json` and now present a condensed research narrative: the supported contribution map, evidence ladder, open-problem proof map, next academic blockers, six decisive metrics, an eighty-two-step visual change trail, a Hardening Map for recent collection-contract fixes, one Evidence Spine for artifact-backed readiness, and a claim-boundary view that separates supported controlled-synthetic results from blocked real-world and Bitcoin attribution claims. Its publication layer includes the post-pack consistency audit so the visible research history does not stop one governance step before the Evidence Pack panel.
...
The latest proof-search track is TICKET-122. It extends TICKET-121 from the first centered pair to every result-independent canonical adjacent shell pair and includes both scalar block means and centered vectors. The exact identity is `|m0|+|m1|-|m0+m1| + w(a+b-p) = 2*1_{m0*m1<0}*min(|m0|,|m1|) + 2w(ab-c)/(a+b+p)`. Two parameter families prove that a perfect first-pair cancellation can be diluted by aligned outer mass and that perfect centered cancellation can be diluted by same-sign scalar means. Across 8 scales, 28 canonical pairs, and 868 pair-denominator rows, the exact total saving is at least `19.3458%`, the joint lower certificate is at least `16.0000%`, reconstruction errors are below `9.32e-10`, and audit failures are zero. Only the finite 8M and 16M rows close; no monotonicity or asymptotic constant is inferred. The corrected target is `VaughanCanonicalPairJointDefectAndResidualBudgetGap`. All four conjectures remain open. See [TICKET122](docs/twin-canonical-joint-defect-audit.md), [TICKET121](docs/twin-balance-angle-defect-audit.md), and [TICKET120](docs/twin-low-divisor-pair-savings-audit.md).

한국어 최신 요약: 최신 탐색은 TICKET-122입니다. 첫 pair의 중심 절감만 보던 TICKET-121을 모든 정준 인접 shell pair의 `mean + centered` 결합 절감으로 확장했습니다. 첫 pair가 완전히 상쇄되어도 바깥 pair의 정렬 질량이 커지면 전역 절감률은 0으로 가고, centered 상쇄가 완전해도 같은 부호 mean이 커지면 전체 절감률은 0으로 간다는 두 반례족을 증명했습니다. 8개 유한 규모, 28개 pair, 868개 pair-denominator 행에서 전역 절감률은 최소 `19.3458%`, 결합 하한 인증률은 최소 `16.0000%`, 검증 실패는 0입니다. 그러나 8M과 16M만 유한 예산이 닫혔고 16M 절감률은 8M보다 낮으므로 점근 단조성이나 양의 극한을 주장하지 않습니다. 다음 목표는 `VaughanCanonicalPairJointDefectAndResidualBudgetGap`이며 네 난제는 모두 미해결입니다.
...
최신 난제 연구 트랙은 TICKET-122입니다. Twin 경로의 모든 정준 인접 shell pair에 대해 스칼라 평균 상쇄와 중심 벡터 상쇄를 하나의 정확한 항등식으로 합쳤고, 첫 pair 또는 centered 성분만 제어하는 경로가 전역 예산을 보장하지 못함을 반례족으로 확인했습니다. 다음 공격은 실제 Vaughan 계수에서 전체 joint saving surplus를 충분히 큰 모든 규모에 대해 양수로 증명하거나, 이를 깨는 Vaughan-realizable 무한 규모열을 구성하는 것입니다. RH, Collatz, Goldbach의 독립 목표는 전이 없이 유지하며 네 문제는 모두 미해결입니다. 문제별 유지·폐기 근거는 [네 난제 통합 연구 정리](docs/open-problem-research-consolidation-2026-07-10.md)에 기록합니다.
