...
The Research Atlas and Project Evolution panels read `data/project_evolution.json` and now present a condensed research narrative: the supported contribution map, evidence ladder, open-problem proof map, next academic blockers, six decisive metrics, a ninety-four-step visual change trail, a Hardening Map for recent collection-contract fixes, one Evidence Spine for artifact-backed readiness, and a claim-boundary view that separates supported controlled-synthetic results from blocked real-world and Bitcoin attribution claims. Its publication layer includes the post-pack consistency audit so the visible research history does not stop one governance step before the Evidence Pack panel.
...
The latest proof-search track is TICKET-134. It replaces loose finite evidence with four exact scale boundaries. For RH, strict finite Gram signs become rational interval semi-decisions after congruence preconditioning, while singular PSD matrices and the infinite Gram tail remain open. For Collatz, every finite or globally bounded-depth contracting-prefix cover is impossible because the exact all-one cylinder stays expanding through the prescribed depth. For Goldbach, a power-of-two spike has an exact detection threshold at moment order comparable to `log(X/log X)`; every sublogarithmic moment regime can still miss it. For Twin Prime, every admissible class has a composite-pair witness below `2Wqr`, extending the residue-only no-go to growing primorial levels `z(X)<=(1-epsilon)log X`. All four conjectures remain open and none is close to proof. See [TICKET134](docs/uniformity-thresholds-and-scale-no-go.md), [TICKET133](docs/quantifier-promotion-exact-reductions.md), and [TICKET132](docs/admissibility-nullset-hard-stratum-local-parity.md).

한국어 최신 요약: 최신 탐색은 TICKET-134입니다. RH에서는 유한 Gram 행렬이 strict positive 또는 indefinite일 때 rational interval과 congruence preconditioner로 유한 인증할 수 있음을 증명했지만 singular PSD와 무한 tail은 남았습니다. Collatz에서는 `n=-1 mod 2^(K+1)`인 all-one valuation cylinder가 깊이 `K`까지 계속 증가하므로 어떤 유한 또는 bounded-depth contracting cover도 전체 자연수를 덮지 못함을 증명했습니다. Goldbach에서는 powers-of-two spike를 검출하려면 moment 차수가 적어도 `log(X/log X)` 규모여야 한다는 정확한 임계값을 얻었습니다. Twin Prime에서는 모든 admissible residue class의 합성수쌍 witness를 `2Wqr` 아래에 구성해, scale과 함께 성장하더라도 `z(X)<=(1-epsilon)log X`인 residue-only 분류기를 배제했습니다. 네 문제는 모두 미해결이며 완전 증명에 가깝다는 근거는 없습니다.
...
최신 난제 연구 트랙은 TICKET-134입니다. [균일성 임계값·규모 의존 no-go 보고서](docs/uniformity-thresholds-and-scale-no-go.md)는 RH의 rational interval Gram dichotomy, Collatz bounded-depth cover 불가능성, Goldbach logarithmic moment 검출 임계값, Twin Prime growing-primorial composite-lift bound를 한글과 영문으로 정리합니다. 다음 공격 대상은 각각 uniform projected Weil Gram tail, well-founded unbounded Collatz cover, log-scale 또는 maximal `K=56` residual, near-full-scale parity-sensitive Twin separation입니다. 네 문제는 모두 미해결입니다.
