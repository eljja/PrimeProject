...
The latest proof-search track is TICKET-116. It lifts every TICKET-115 endpoint coefficient into the actual positive and negative outer-Möbius layers of Vaughan's Type-II identity and proves the exact centered polarization ledger. Bounding the two sign layers independently worsens the numerator budget on all six scales and closes none: at 4M it inflates the budget from `1.450M` to `4.187M` and changes the finite lower expression from `+335,523.7` to `-2,401,998.7`. The centered covariance is positive on only four of six rows, so no favorable-sign assumption is promoted. The next target is `EventuallySubcriticalSignedVaughanMobiusCyclotomicDispersionBudget`. These remain finite audits, not a Twin Prime proof. See [TICKET116](docs/twin-mobius-sign-cyclotomic-audit.md), [TICKET115](docs/twin-complex-cyclotomic-dispersion-audit.md), and [TICKET114](docs/twin-ramanujan-numerator-dispersion-audit.md).

한국어 최신 요약: 최신 탐색은 TICKET-116입니다. 실제 Vaughan Type-II 계수를 바깥 Möbius 부호에 따라 양의 층과 음의 층으로 분해하고, Farey endpoint 평균과 중심화 벡터에서도 원래 계수가 두 층의 차이로 정확히 재구성됨을 검증했습니다. 두 부호층을 각각 절댓값 처리하면 여섯 규모 모두 악화되며, 4M 하한은 `+335,523.7`에서 `-2,401,998.7`로 무너집니다. 중심화 공분산도 처음 두 규모에서는 음수이므로 유리한 부호를 가정할 수 없습니다. 따라서 독립 부호층 삼각부등식은 폐기하고 signed Möbius 상호작용을 보존하는 `EventuallySubcriticalSignedVaughanMobiusCyclotomicDispersionBudget`만 유지합니다. 네 난제는 모두 여전히 미해결입니다.
