...
The latest proof-search track is TICKET-57, a parametric boundary-state audit. TICKET56 closed the exact 32-bit extracted lasso route as a finite partition, then refuted simple projection closure with a sampled 48-bit depth-15 witness. TICKET57 now attacks the next shortcut: within the exact 32-bit population, the extracted template alone has 6 different coarse outcomes, and even `template + prefix_length + residue mod 2^26` still leaves 92 collision groups. The first deterministic exact32 boundary in the audited ladder is `template + prefix_length + residue mod 2^28`. Known near-lasso roots replay at most depth 15, and 0 replay a full lasso period. This is still not a Collatz proof or counterexample; it narrows the next obligation to an affine-boundary lift theorem or a new full-period escape witness.

한국어 최신 요약: 최신 proof-search는 TICKET-57입니다. TICKET56이 exact 32-bit start-template 전체를 finite partition으로 닫고, 48-bit projection escape 때문에 simple projection-closure 귀납이 틀렸음을 보였다면, TICKET57은 그 다음 약한 지점인 “유한 template automaton만으로 충분한가”를 공격합니다. exact32 내부에서도 template만 보면 coarse outcome이 6개로 갈라지고, `template + prefix_length + residue mod 2^26`까지 추가해도 collision group이 92개 남습니다. 이번 계산에서 처음으로 exact32 분기가 결정되는 경계는 `template + prefix_length + residue mod 2^28`입니다. 알려진 near-lasso root 3개는 최대 depth 15까지만 재생되고, full lasso period를 재생한 경우는 0개입니다. 따라서 이것은 콜라츠 증명이나 반례가 아니라, 다음 증명 의무를 affine-boundary lift theorem 또는 full-period escape witness로 더 좁힌 결과입니다.
...
최신 난제 연구 트랙은 TICKET-57입니다. 기존 TICKET49의 all-phase next-valuation 장애물은 TICKET50에서 32비트 예외로 반박됐고, TICKET51/TICKET52는 32비트와 48비트 near-lasso 후보를 terminal branch에서 닫았습니다. TICKET53은 현재 추출된 phase-15 lasso family가 terminal target에 도달할 수 없다는 상징 terminal mismatch theorem으로 정리했고, TICKET54는 남은 후보를 phase-5 gate로 압축했습니다. TICKET55는 gate crosser tunnel을 닫았고, TICKET56은 exact32 lasso route 전체를 finite partition으로 닫으면서도 simple projection-closure 귀납이 틀린 경로임을 분리했습니다. TICKET57은 exact32 내부 결정성에도 `residue mod 2^28` 수준의 boundary coordinate가 필요함을 보여주고, 알려진 near-lasso root 중 full-period replay가 없음을 확인했습니다. 다음 목표는 이 affine-boundary state가 40/48비트 lift에서도 안정적인지 증명하거나, 안정성을 깨는 full-period escape witness를 찾는 것입니다.
