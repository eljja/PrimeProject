...
The latest proof-search track is TICKET-46, a stable-clause grammar stress test. It pushes the Collatz lift-template route to 28 bits and refutes all five tested finite template-local scalar clause-rank grammars by nonnegative-pressure cycles, including the exact observed-template table. This is a restricted no-go result for those proof routes, not a Collatz proof or counterexample; the remaining target is a horizon-independent ordinal/stateful measure or a future counteredge against that stronger object.
...
최신 난제 연구 트랙은 TICKET-46입니다. Collatz lift-template 경로를 28-bit까지 밀어 올려, 테스트한 다섯 개의 finite template-local scalar clause-rank grammar가 모두 nonnegative-pressure cycle로 깨진다는 것을 확인했습니다. 여기에는 26-bit에서 bounded ceiling certificate처럼 보였던 exact observed-template table도 포함됩니다. 이 결론은 Collatz 증명이나 반례가 아니라, 해당 제한된 증명 전략들이 더 이상 남아 있지 않다는 no-go 결과입니다. 다음 목표는 horizon에 의존하지 않는 ordinal/stateful measure를 만들거나, 그 더 강한 객체에 대한 미래 counteredge를 찾는 것입니다.
