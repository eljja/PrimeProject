...
The Research Atlas and Project Evolution panels read `data/project_evolution.json` and now present a condensed research narrative: the supported contribution map, evidence ladder, open-problem proof map, next academic blockers, six decisive metrics, an eighty-four-step visual change trail, a Hardening Map for recent collection-contract fixes, one Evidence Spine for artifact-backed readiness, and a claim-boundary view that separates supported controlled-synthetic results from blocked real-world and Bitcoin attribution claims. Its publication layer includes the post-pack consistency audit so the visible research history does not stop one governance step before the Evidence Pack panel.
...
The latest proof-search track is TICKET-124. It replaces the sufficient-but-unnecessarily-strong TICKET-123 ratio triple with the exact joint obstruction `Q_X=((S_X-D_X)+E_X)/K_X`. It proves `EventualPositiveMarginIffLimsupObstructionBelowOne`: a fixed positive normalized margin exists at every sufficiently large scale if and only if `limsup Q_X<1`. An alternating compensation sequence has `Q_X=0.8` and margin `0.2` at every scale while every coordinatewise TICKET-123 envelope has compatibility left side at least `1.6`; this proves that the old modular target was not necessary. A second sequence closes with `D_X/S_X=0`, and endpoint and finite-prefix families prove strictness and block finite-to-infinite inference. The corrected Twin target is `VaughanCanonicalObstructionLimsup`, with exact-gap transfer and parity survival still open. RH kernel positivity now requires an exact all-test-function contract; Collatz golden-mean escape is demoted to a Mersenne-route lemma while `ResidueRankDescentCover` returns as the global target; Goldbach requires an explicit residual constant, cutoff, major term, and finite glue. All four conjectures remain open. See [TICKET124](docs/canonical-obstruction-limsup-criterion.md), [TICKET123](docs/canonical-defect-ratio-closure-bridge.md), and [TICKET122](docs/twin-canonical-joint-defect-audit.md).

한국어 최신 요약: 최신 탐색은 TICKET-124입니다. 기존 `eta-rho-epsilon` 세 상수 목표는 충분조건이지만 필요조건이 아니었습니다. 스케일별 pair 잔여항과 경계항의 보상을 보존하는 `Q_X=((S_X-D_X)+E_X)/K_X`를 정의하고, 충분히 큰 모든 규모에서 고정 양의 여유가 존재하는 것과 `limsup Q_X<1`이 정확히 동치임을 증명했습니다. 실제 결합 `Q`는 항상 0.8인데 좌표별 최악 상한은 1.6이 되는 교대 수열로 이전 목표의 필요성을 반박했습니다. 이는 Twin Prime 반례가 아니라 과도하게 강한 보조목표의 반례입니다. 다음 목표는 실제 Vaughan 예산의 `limsup Q_X<1`을 산술적으로 증명하거나 `Q_X>=1`인 무한 부분수열을 구성하는 것입니다. Collatz의 황금평균 탈출은 전체 추측의 직접 연결이 아닌 Mersenne 경로 보조명제로 강등했고, RH와 Goldbach도 정확한 무한 연결 계약을 강화했습니다. 네 난제는 모두 미해결입니다.
...
최신 난제 연구 트랙은 TICKET-124입니다. Twin의 선택된 정준 예산에서 결합 장애량 `Q_X=((S_X-D_X)+E_X)/K_X`를 정의하고, 고정 양의 점근 여유와 `limsup Q_X<1`의 필요충분성을 증명했습니다. 좌표별 최악값을 따로 취하는 이전 세 비율 목표가 보상 구조를 잃는다는 정확한 반례도 추가했습니다. 다음 공격은 실제 Vaughan 계수의 결합 `Q_X`를 dyadic block에서 직접 제어하는 것이며, 유한 8개 행의 추세 외삽은 허용하지 않습니다. RH는 exact test class positivity, Collatz는 universal residue-rank descent, Goldbach는 explicit cutoff bridge를 직접 목표로 삼습니다. 네 문제는 모두 미해결입니다. 문제별 유지·폐기 근거는 [TICKET-124 논리 교정](docs/canonical-obstruction-limsup-criterion.md)과 [네 난제 통합 연구 정리](docs/open-problem-research-consolidation-2026-07-10.md)에 기록합니다.
