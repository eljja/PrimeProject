...
degree-five polynomial decides every q-adic depth of the fixed Collatz
32/27 difference. Rational-center Goldbach residual energy is exactly a
prime-residue discrepancy norm. The odd prime-power pair proxy has an explicit
composite-power contamination bound and is not equal to the twin count.
The nine moment rows, 17,981-prime replay, 186 residue-denominator scans, and
six twin scales are bounded evidence only. All parent conjectures remain
`open_not_proven`.
...
한국어 최신 요약: 최신 탐색은 TICKET-246입니다. 리만 트랙은 모든 고정 유한
짝수 모멘트 목록의 명시적 정규화 소거자를 구성해 그 분리 경로를 no-go로
닫았습니다. 콜라츠 트랙은 고정기저 차의 모든 q진 깊이를 판정하는 유한
다항식 항등식을 증명했습니다. 골드바흐 트랙은 유리 중심 잔차의 정확한
Parseval 노름을, 쌍둥이 소수 트랙은 소수거듭제곱 프록시 오염 상계를
증명했습니다. 이들은 세 partial theorem과 한 exact no-go이며 유한 계산은
선언 범위의 재현 증거일 뿐입니다.
네 상위 문제의 해결 수는 0입니다.
...
최신 난제 연구 트랙은 TICKET-246입니다.
[유한 모멘트·전 깊이 Fermat 다항식·유리 중심 Parseval·소수거듭제곱 오염 한국어 보고서 (parent conjecture claims blocked · open_not_proven)](docs/moment-alldepth-parseval-primepower.ko.md)와
[영문 보고서](docs/moment-alldepth-parseval-primepower.md)는
네 트랙의 정확 명제, 증명, 재현 계산, 폐기 경로와 남은 무한 간극을
분리합니다. 현재 결정적 미해결 보조정리는 실제 Weil closure의 infinite-
feature coercivity, 고정 밑 Pq와 Uq−Vq의 전소수 valuation 지배, quarter-
torus 잉여류 분산 감쇠와 arc stability, 소수거듭제곱 오염을 이기는
scale-local Type II 하한입니다.

주요 진입점:

- [메인 연구실](https://eljja.github.io/PrimeProject/?lang=ko): TICKET-246의 현재 연구 경계, 네 문제의 부분정리·폐기 경로·남은 간극을 먼저 보고 소수 탐색기와 암호 fingerprint 실험으로 이동합니다.
- [Proof Workbench](https://eljja.github.io/PrimeProject/open-problems/index.html): 네 문제의 현재 보조정리를 한 화면에서 비교하며, 246개 티켓은 네 연구 시대와 재현 기록으로 압축해 탐색합니다.
