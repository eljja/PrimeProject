...
The Research Atlas and Project Evolution panels read `data/project_evolution.json` and now present a condensed research narrative: the supported contribution map, evidence ladder, open-problem proof map, next academic blockers, six decisive metrics, a seventy-step visual change trail, a Hardening Map for recent collection-contract fixes, one Evidence Spine for artifact-backed readiness, and a claim-boundary view that separates supported controlled-synthetic results from blocked real-world and Bitcoin attribution claims. Its publication layer includes the post-pack consistency audit so the visible research history does not stop one governance step before the Evidence Pack panel.
...
The latest proof-search track is TICKET-110. It fixes rational major arcs around every reduced `a/q`, `q<=Q`, before reading target contributions and partitions the spectrum exactly. At `X=1,048,576`, `Q=32` gives major contribution `461,203.6` and actual minor contribution `-2,063.7`, reconstructing exact correlation `459,139.9`; the trivial minor-energy bound is nevertheless `-3,105,699.1`. The next target is `FixedBumpMajorArcAsymptoticWithTypeIIMinorPowerSaving`. See [TICKET110](docs/twin-rational-arc-budget.md), [TICKET109](docs/twin-spectral-phase-audit.md), and [TICKET108](docs/twin-joint-equivalence-smoothing.md).

한국어 최신 요약: 최신 탐색은 TICKET-110입니다. 사전에 고정한 모든 유리수 `a/q`, `q<=Q` major arc로 스펙트럼을 정확히 분할했습니다. 1,048,576에서 `Q=32` major 기여 `461,203.6`과 실제 minor 기여 `-2,063.7`이 정확한 상관 `459,139.9`를 재구성하지만, trivial minor 하한은 `-3,105,699.1`이라 증명이 되지 않습니다. 다음 목표는 `FixedBumpMajorArcAsymptoticWithTypeIIMinorPowerSaving`이며 네 난제는 아직 미해결입니다.
