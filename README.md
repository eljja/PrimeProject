...
It proves that no finite real even Taylor jet alone can certify the all-real-zero
property, while preserving Rouché tail control as a valid conditional bridge.
It proves every fixed one-count/rest-two Collatz stratum decidable and excludes
the complete eleven-one stratum, representing 3,151,735,808 normalized words in
the finite boundary audit. It also splits Goldbach and Twin Prime
proper-prime-power contamination into a dominant prime-square layer and a
cubic-scale higher-exponent tail. These are exact partial results and no-go
boundaries, not resolutions. Every conjecture remains `open_not_proven`; the
...
The Research Atlas and Project Evolution panels read `data/project_evolution.json` and present a condensed research narrative: the supported contribution map, evidence ladder, open-problem proof map, academic blockers, six decisive metrics, a 156-step visual change trail, a Hardening Map, one Evidence Spine, and a claim-boundary view that separates supported controlled-synthetic results from blocked real-world and Bitcoin attribution claims.
...
The latest proof-search track is TICKET-195. It closes the complete
exactly-eleven-one/rest-two accelerated Collatz cycle stratum for all horizons:
noncontraction covers `h<=26`, an exact 5+5 meet-in-the-middle residue audit
represents 3,151,735,808 normalized words at `h=27..41`, and
`2048(5/6)^h<1` closes `h>=42`. The RH track proves a finite-even-jet ambiguity
theorem and retains strict Rouché tail control as the valid bounded-domain
bridge, without proving the actual Xi tail bound. Goldbach and Twin Prime split
proper-prime-power contamination into prime-square leading layers and
`O(X^(1/3) log X)` higher-layer contamination. These are partial theorems and
route corrections, not solutions. All four conjectures remain open and the
machine resolution count is zero.
See
...
한국어 최신 요약: 최신 탐색은 TICKET-195입니다. 콜라츠 트랙은 정확히 열한
번의 `v=1`과 나머지 `v=2`를 갖는 모든 주기족을 전 길이에서 배제하고, 각
고정 one-count 층이 유한 결정 가능함을 증명했습니다. 리만 트랙은 유한
짝함수 제트만으로 모든 영점의 실수성을 승격할 수 없다는 반례 구성을
증명했으며 실제 Xi에는 Rouché 꼬리 인증이 남아 있습니다. 골드바흐와
쌍둥이 소수는 소수 제곱 주도층과 세제곱 규모 고차 꼬리를 분리했습니다.
실제 무한 전제는 남아 있고 네 문제의 해결 수는 0입니다.
...
최신 난제 연구 트랙은 TICKET-195입니다.
[유한 제트 경계·열한-1 결정 가능성·소수 제곱층 한국어 보고서](docs/finitejet-elevenone-squarelayer.ko.md)와
[영문 보고서](docs/finitejet-elevenone-squarelayer.md)는 네 트랙의
정확 명제, 증명, 재현 계산, 폐기 경로와 남은 무한 간극을 분리합니다.
열한 개 `v=1` 주기족 전체를 새로 닫았지만 실제 무한 전제는 남아 있습니다.
각 proof DAG는 TICKET-194 입력, 이번에 닫힌 정리, 폐기된 승격 경로,
다음 단일 미증명 보조정리를 분리합니다.

주요 진입점:

- [메인 연구실](https://eljja.github.io/PrimeProject/?lang=ko): TICKET-195의 현재 연구 경계, 네 문제의 부분정리·폐기 경로·남은 간극을 먼저 보고 소수 탐색기와 암호 fingerprint 실험으로 이동합니다.
- [Proof Workbench](https://eljja.github.io/PrimeProject/open-problems/index.html): 네 문제의 현재 보조정리를 한 화면에서 비교하며, 195개 티켓은 네 연구 시대와 재현 기록으로 압축해 탐색합니다.
