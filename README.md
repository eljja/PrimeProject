...
with a separate [Korean report](docs/sharp-envelope-diagonal-mod32-ninthorder.ko.md).
It proves four partial theorems. An RH-style reciprocal error envelope gives
the sharp lag bound `L-2A`; all fixed Weyl limits are equivalent to uniform
cancellation on some growing cutoff; a special mod-3 Goldbach tie forces one
of four level-phased residues modulo 32; and, beyond an explicit root-cone
threshold, the Twin joint ninth-order congruence pair is equivalent to the
degree-17 unit equation. All four parent conjectures remain
`open_not_proven`.
...
한국어 최신 요약: 최신 탐색은 TICKET-263입니다. 리만 트랙은 reciprocal
envelope의 sharp 상수 2를 증명했고, 콜라츠 트랙은 모든 고정 Weyl 극한과
어떤 증가 cutoff의 균일 상쇄가 동치임을 증명했습니다. 골드바흐 트랙은
특수 mod-3 tie의 mod-32 네 위상을 확정했고 실제 세 prefix를 독립 계산으로
배제했습니다. 쌍둥이 소수 트랙은 명시적 root cone 문턱 뒤에서 mod `v^9`와
mod `u^9` 양방향 조건이 원래 unit 방정식과 동치임을 증명했습니다.
...
최신 난제 연구 트랙은 TICKET-263입니다.
[sharp envelope·증가 Weyl cutoff·mod-32 위상·9차 tail exactness 한국어 보고서 (parent conjecture claims blocked · open_not_proven)](docs/sharp-envelope-diagonal-mod32-ninthorder.ko.md)와
[영문 보고서](docs/sharp-envelope-diagonal-mod32-ninthorder.md)는 네 트랙의 정확 명제,
증명, 재현 계산, 폐기 경로와 남은 무한 간극을 분리합니다. 현재 결정적
미해결 보조정리는 actual Weil packet의 reciprocal envelope가 limit의
절반보다 작다는 추정, canonical Fermat-quotient growing-cutoff 균일 상쇄,
특수 지수에서 level-phased mod-32 잔여 회피, 모든 unique-root convergent에서
양방향 9차 합동식의 동시 실패입니다. 주요 진입점:

- [메인 연구실](https://eljja.github.io/PrimeProject/?lang=ko): TICKET-263의 현재 연구 경계, 네 문제의 부분정리·폐기 경로·남은 간극을 먼저 보고 소수 탐색기와 암호 fingerprint 실험으로 이동합니다.
- [Proof Workbench](https://eljja.github.io/PrimeProject/open-problems/index.html): 네 문제의 현재 보조정리를 한 화면에서 비교하며, 263개 티켓은 네 연구 시대와 재현 기록으로 압축해 탐색합니다.
