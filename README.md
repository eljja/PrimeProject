...
The Research Atlas and Project Evolution panels read `data/project_evolution.json` and now present a condensed research narrative: the supported contribution map, evidence ladder, open-problem proof map, next academic blockers, six decisive metrics, a one-hundred-step visual change trail, a Hardening Map for recent collection-contract fixes, one Evidence Spine for artifact-backed readiness, and a claim-boundary view that separates supported controlled-synthetic results from blocked real-world and Bitcoin attribution claims. Its publication layer includes the post-pack consistency audit so the visible research history does not stop one governance step before the Evidence Pack panel.
...
The latest proof-search track is TICKET-140. For RH it gives a cancellation-sensitive even-trace spectral certificate and proves that fixed-factor worst-case control requires moment order proportional to log dimension. For Collatz it proves that every fixed minimum-cycle floor, including `2^28`, makes the TICKET139 product window eventually vacuous; the next route must use a period-dependent minimum. For Goldbach it characterizes pointwise control by an exact measurement row-space dual and proves that incomplete moments on powers of two leave every coordinate unbounded. For Twin Prime it proves sample-length-uniform Sobolev cancellation for unweighted `sqrt(2)` rotations while isolating the missing arithmetic Type II coefficients and positive gap-two mass. All four conjectures remain open. See [TICKET140](docs/spectral-moments-fixed-floor-duality-and-rotation.md), [TICKET139](docs/uniformity-diophantine-and-complexity.md), and [TICKET138](docs/correlation-periodicity-and-scale-closure.md).
