...
The Research Atlas and Project Evolution panels read `data/project_evolution.json` and now present a condensed research narrative: the supported contribution map, evidence ladder, open-problem proof map, next academic blockers, six decisive metrics, an eighty-five-step visual change trail, a Hardening Map for recent collection-contract fixes, one Evidence Spine for artifact-backed readiness, and a claim-boundary view that separates supported controlled-synthetic results from blocked real-world and Bitcoin attribution claims. Its publication layer includes the post-pack consistency audit so the visible research history does not stop one governance step before the Evidence Pack panel.
...
The latest proof-search track is TICKET-125. It proves four exact conditional bridges and records the still-open arithmetic premises. RH positivity extends from a dense cone only after density, continuity, and non-circular all-cone positivity are proved; exact countermodels show that none may be omitted. For Collatz, `UniversalFiniteStoppingDescentIffCollatz` is proved by strong induction, and an exact lifted-cylinder audit certifies uniform descent for 121,825 of 131,072 odd residue classes modulo `2^18`; 9,247 remain unresolved, with a refinement boundary at every fixed precision. Goldbach now has the explicit endpoint condition `A-K/log(H)-B log(H)^2/sqrt(H)>0` at the verified `H=4e18`; the constants are not proved. Twin Prime now has a dyadic contraction bridge: `Q_(2X)<=3Q_X/4+23/100` uniformly would imply dyadic `limsup Q<=0.92`, but the four passing finite transitions are selected candidate data without a holdout or tail theorem. All four conjectures remain open. See [TICKET125](docs/infinite-bridge-contracts.md), [TICKET124](docs/canonical-obstruction-limsup-criterion.md), and [TICKET123](docs/canonical-defect-ratio-closure-bridge.md).

한국어 최신 요약: 최신 탐색은 TICKET-125입니다. 네 난제마다 조건부 무한 연결 정리와 약한 전제가 실패하는 정확한 반례를 만들었습니다. RH는 조밀성·연속성·비순환 원뿔 양성이 모두 필요합니다. Collatz는 모든 수가 유한 단계에서 자기보다 작아지면 강한 귀납법으로 추측이 따라온다는 동치를 증명했고, `2^18` 홀수 잉여류 131,072개 중 121,825개 원통 전체의 하강을 기호적으로 인증했습니다. 9,247개와 매 고정 정밀도의 경계 원통은 남아 있습니다. Goldbach는 `4*10^18` 계산과 무한 꼬리를 잇는 명시적 끝점 부등식을 도출했지만 해석 상수는 미증명입니다. Twin은 `Q_(2X)<=3Q_X/4+23/100`의 균일한 증명이 dyadic `limsup Q<=0.92`를 준다는 수축 정리를 증명했지만, 현재 네 전이는 후보 생성 자료일 뿐입니다. 네 난제는 모두 미해결입니다.
...
최신 난제 연구 트랙은 TICKET-125입니다. RH의 연속 조밀 원뿔 확장, Collatz의 보편 유한 하강 동치와 적응형 잉여류 원통 인증, Goldbach의 `4*10^18` 끝점 예산, Twin의 dyadic affine 수축을 각각 정확한 정리로 분리했습니다. Collatz 18비트 감사는 무한히 많은 정수를 포함하는 121,825개 원통을 인증하지만 9,247개를 남깁니다. Twin의 네 전이 통과도 무한 외삽에 사용하지 않습니다. 다음 공격은 [TICKET-125 무한 연결 계약](docs/infinite-bridge-contracts.md)에 정의된 문제별 TICKET-126 목표이며, 네 문제는 모두 미해결입니다.
