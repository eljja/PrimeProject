...
The latest proof-search track is TICKET-58, an affine-boundary lift audit. TICKET57 found that the first deterministic exact32 boundary is `template + prefix_length + residue mod 2^28`. TICKET58 replays the deterministic TICKET52 48-bit sample against that boundary. Among 3,184 sampled 48-bit start-template matches, 3,086 project outside the exact32 target. Only 98 project into the exact32 target, and 70 of those disagree with the exact32 boundary-predicted outcome. No sampled root replays a full lasso period. This refutes the shortcut “exact32 deterministic boundary lifts unchanged,” but it is still not a Collatz proof or counterexample.

한국어 최신 요약: 최신 proof-search는 TICKET-58입니다. TICKET57에서 처음으로 결정적이었던 exact32 경계는 `template + prefix_length + residue mod 2^28`였습니다. TICKET58은 같은 TICKET52 deterministic 48-bit sample을 재생해 이 boundary가 lift에서도 유지되는지 검사했습니다. 48-bit start-template match `3,184`개 중 `3,086`개는 exact32 target 밖으로 projection escape했고, exact32 target 안으로 들어온 `98`개 중 `70`개는 exact32 boundary가 예측한 outcome과 실제 48-bit outcome이 달랐습니다. full lasso period를 재생한 샘플은 0개입니다. 따라서 “exact32 deterministic boundary를 그대로 lift하면 된다”는 경로는 샘플에서 반박됐지만, 이것은 여전히 콜라츠 증명이나 반례가 아닙니다.
...
최신 난제 연구 트랙은 TICKET-58입니다. 기존 TICKET49의 all-phase next-valuation 장애물은 TICKET50에서 32비트 예외로 반박됐고, TICKET51/TICKET52는 32비트와 48비트 near-lasso 후보를 terminal branch에서 닫았습니다. TICKET53은 현재 추출된 phase-15 lasso family가 terminal target에 도달할 수 없다는 상징 terminal mismatch theorem으로 정리했고, TICKET54는 남은 후보를 phase-5 gate로 압축했습니다. TICKET55는 gate crosser tunnel을 닫았고, TICKET56은 exact32 lasso route 전체를 finite partition으로 닫으면서도 simple projection-closure 귀납이 틀린 경로임을 분리했습니다. TICKET57은 exact32 결정성에 `residue mod 2^28` boundary가 필요함을 보였고, TICKET58은 그 boundary가 48비트 샘플 lift에서 보존되지 않는 mismatch를 찾았습니다. 다음 목표는 projection escape와 lift mismatch를 더 이상 샘플이 아니라 symbolic cylinder theorem으로 승격하는 것입니다.
