...
The Research Atlas and Project Evolution panels read `data/project_evolution.json` and now present a condensed research narrative: the supported contribution map, evidence ladder, open-problem proof map, next academic blockers, six decisive metrics, an eighty-seven-step visual change trail, a Hardening Map for recent collection-contract fixes, one Evidence Spine for artifact-backed readiness, and a claim-boundary view that separates supported controlled-synthetic results from blocked real-world and Bitcoin attribution claims. Its publication layer includes the post-pack consistency audit so the visible research history does not stop one governance step before the Evidence Pack panel.
...
The latest proof-search track is TICKET-127. It publicly repairs TICKET-126's omitted Collatz base exception and proves four exact intermediate bridges. RH counterexample search becomes a semidecision only under an exact continuous Weil criterion, an enumerable dense core, and complete strict-sign interval evaluation; finite non-discovery proves nothing. Collatz is equivalent to excluding **nontrivial** eventually-low paths: at 28 bits, 4,027,109 of 134,217,728 odd classes remain after removing the permanent `n=1` path, and the longest nontrivial low-run witnesses are `27` and `31`. Goldbach's binary singular series satisfies `S(N)>=1`, closing normalized `A=1` in the exact residual identity while leaving the pointwise residual theorem `K<42.83274372223497` open. Twin's normalized recurrence is now reduced exactly to a raw Vaughan numerator/denominator transport inequality; the 16M-to-32M audit has numerator growth `1.8603305083667954` versus denominator growth `2.0115420952456007`, but no uniform tail, interpolation, parity, or exact-gap theorem follows. All four conjectures remain open. See [TICKET127](docs/exception-repair-and-effective-bridges.md), [TICKET126](docs/route-correction-and-premise-closure.md), and [TICKET125](docs/infinite-bridge-contracts.md).

한국어 최신 요약: 최신 탐색은 TICKET-127입니다. RH에서는 조밀한 Weil 핵심집합과 엄밀한 구간 계산기가 있을 때 반례 탐색이 반결정 절차가 됨을 증명했지만, 유한 비발견을 RH 증명으로 승격하지 않습니다. Collatz에서는 TICKET-126이 `n=1` 예외를 누락한 부분을 공개 교정했습니다. 28비트의 실제 반례 후보는 `n=1`을 뺀 4,027,109개이며, 최장 비자명 low-run 증인은 `27`, `31`입니다. Goldbach 특이급수의 `S(N)>=1`로 정확한 잔차 정규화의 `A=1`을 닫았지만, 전칭 점별 잔차 `K<42.83274372223497`가 핵심 미증명입니다. Twin은 정규화 점화식을 원시 Vaughan 예산 운송 부등식으로 정확히 환원했으나 균일 꼬리, 보간, parity, 간격 2 정리는 없습니다. 네 난제는 모두 미해결입니다.
...
최신 난제 연구 트랙은 TICKET-127입니다. [예외 교정 및 실효적 증명 연결 보고서](docs/exception-repair-and-effective-bridges.md)는 RH 반례 반결정 정리, Collatz `n=1` 예외 교정, Goldbach 특이급수 하한, Twin 원시 예산 운송 동치를 한글과 영문으로 정리합니다. 다음 공격 대상은 각각 구간 인증 Weil 핵심 계산기, 비자명 결국-low 경로의 균일 배제, 명시적 점별 Goldbach 잔차, 균일 Vaughan 원시 예산 운송·보간입니다. 네 문제는 모두 미해결입니다.
