...
The Research Atlas and Project Evolution panels read `data/project_evolution.json` and now present a condensed research narrative: the supported contribution map, evidence ladder, open-problem proof map, next academic blockers, six decisive metrics, an eighty-six-step visual change trail, a Hardening Map for recent collection-contract fixes, one Evidence Spine for artifact-backed readiness, and a claim-boundary view that separates supported controlled-synthetic results from blocked real-world and Bitcoin attribution claims. Its publication layer includes the post-pack consistency audit so the visible research history does not stop one governance step before the Evidence Pack panel.
...
The latest proof-search track is TICKET-126. It corrects two overbroad routes, proves three exact intermediate theorems, and reports one preregistered finite holdout. RH's autocorrelation cone is separated from any negative-at-identity test function, so full-space density is discarded while direct admissible Weil positivity remains open. Collatz finite-stopping counterexamples are exactly eventually-low paths in the adaptive unresolved tree; the exact 28-bit audit leaves 4,027,110 of 134,217,728 odd classes unresolved, and the non-natural 2-adic boundary ray is retired. Goldbach's proper-prime-power contamination premise is now closed with uniform `B=2.0949181787429647` above `H=4e18`; explicit major and residual constants `A,K` remain open. Twin's frozen 16M-to-32M holdout passes with certified residual `0.145872900933948<0.23`, but five finite transitions prove no tail recurrence, interpolation theorem, parity breakthrough, or gap-two lower bound. All four conjectures remain open. See [TICKET126](docs/route-correction-and-premise-closure.md), [TICKET125](docs/infinite-bridge-contracts.md), and [TICKET124](docs/canonical-obstruction-limsup-criterion.md).

한국어 최신 요약: 최신 탐색은 TICKET-126입니다. RH에서는 자기상관 원뿔이 항등원 값이 음수인 시험함수를 포함한 전체 공간에 조밀할 수 없음을 증명하여 이 우회로를 폐기했고, 정확한 허용 자기상관 전체의 비순환 Weil 양성을 다음 목표로 남겼습니다. Collatz에서는 자연수 유한 하강 반례가 적응형 미해결 트리의 `결국-low` 무한 경로와 정확히 동치임을 증명했습니다. 28비트 감사에서 홀수 잉여류 134,217,728개 중 4,027,110개가 남지만 이 유한 비율을 무한 결론으로 외삽하지 않습니다. Goldbach에서는 진소수 거듭제곱 오염 상수 `B=2.0949181787429647`을 명시적으로 닫았고 주항·잔차 상수 `A,K`가 남았습니다. Twin은 사전 고정한 16M→32M 검사를 통과했지만 이는 다섯 번째 유한 전이일 뿐입니다. 네 난제는 모두 미해결입니다.
...
최신 난제 연구 트랙은 TICKET-126입니다. [경로 교정 및 전제 폐쇄 보고서](docs/route-correction-and-premise-closure.md)는 RH 전체공간 조밀성 경로의 정확한 실패, Collatz 자연수 경로의 `결국-low` 동치, Goldbach 진소수 거듭제곱 상수 `B`의 폐쇄, Twin 32M 사전등록 홀드아웃을 한글과 영문으로 정리합니다. 다음 공격 대상은 각각 비순환 Weil 자기상관 양성, 결국-low 경로의 균일 배제, 명시적 Goldbach `A,K`, Vaughan 장애량의 균일 수축·보간입니다. 네 문제는 모두 미해결입니다.
