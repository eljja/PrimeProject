...
The latest proof-search track is TICKET-132. It corrects the mathematical space used by the RH route and proves exact pointwise-boundary countermodels for the other three routes. For RH, a continuous effective projection turns the rational-bump core into a countable dense core satisfying both exact Weil moment conditions. For Collatz, natural valuation codes are simultaneously countable, dense in the product topology, null under every non-atomic measure, and eventually residue-stabilizing; topology or density alone therefore cannot settle the pointwise problem. For Goldbach, the unbounded family `N=2^m` always has normalized singular-series multiplier one, so finite small-divisor stratification cannot remove the uniform hard stratum. For Twin Prime, CRT constructs infinitely many small-prime-clean pairs whose two entries are both composite at every fixed sieve level. All four conjectures remain open and none is close to proof. See [TICKET132](docs/admissibility-nullset-hard-stratum-local-parity.md), [TICKET131](docs/proof-viability-and-target-correction.md), and [TICKET130](docs/computability-cap-language-optimality.md).

한국어 최신 요약: 최신 탐색은 TICKET-132입니다. RH에서는 임의의 유리 bump가 실제 Weil 허용조건을 만족한다고 보던 결함을 고쳐, 두 모멘트 제약을 정확히 보존하는 열거 가능 조밀 core를 구성했습니다. Collatz에서는 자연수 valuation code가 가산이면서 모든 유한 prefix에 조밀하고 비원자 측도에서는 측도 0임을 증명했습니다. 따라서 finite-prefix 분리와 확률적 mass 감소는 모두 점별 자연수 하강을 대신하지 못합니다. Goldbach에서는 `2^m`이 항상 최소 특이급수 multiplier `1`을 갖는 무한 hard stratum임을 증명했습니다. Twin Prime에서는 모든 고정 sieve 수준을 통과하면서 `n`과 `n+2`가 모두 합성수인 무한 CRT 산술진행을 구성했습니다. 네 문제는 모두 미해결이며 완전 증명에 가깝다는 근거는 없습니다.
...
최신 난제 연구 트랙은 TICKET-132입니다. [허용 core·dense-null code·hard stratum·local parity 보고서](docs/admissibility-nullset-hard-stratum-local-parity.md)는 TICKET-131까지의 유효 결과를 유지하면서 RH core의 모멘트 제약을 수리하고, Collatz의 topology/measure 한계, Goldbach powers-of-two hard stratum, Twin Prime finite-local sieve의 CRT countermodel을 한글과 영문으로 정리합니다. 다음 공격 대상은 각각 projected Weil core의 전칭 비음성, 자연수 code의 점별 Archimedean descent, powers-of-two와 rough 짝수의 점별 `K=56` residual, unbounded Type II 정보를 사용하는 parity-sensitive exact-gap certificate입니다. 네 문제는 모두 미해결입니다.
