...
It excludes every accelerated Collatz cycle word containing exactly seven
valuation-one entries and otherwise only valuation two. It also proves a
Gaussian-rational scalar-probe promotion boundary for Riemann, reduces the
Goldbach bridge to an exact sublinear pointwise prime-power budget, and proves
that positive prime-power-subtracted Twin block excess is equivalent to a twin
pair in that block. These are exact partial results and no-go boundaries, not
...
The Research Atlas and Project Evolution panels read `data/project_evolution.json` and present a condensed research narrative: the supported contribution map, evidence ladder, open-problem proof map, academic blockers, six decisive metrics, a 152-step visual change trail, a Hardening Map, one Evidence Spine, and a claim-boundary view that separates supported controlled-synthetic results from blocked real-world and Bitcoin attribution claims.
...
The latest proof-search track is TICKET-191. It closes the complete
exactly-seven-one/rest-two accelerated Collatz cycle stratum for all horizons:
the product bound `128(5/6)^h<1` covers `h>=27`, and exact integer enumeration
closes 2,195,765 contracting words at `h=17..26`. The RH track proves a
countable scalar-probe promotion theorem and a coordinate-test counterexample.
Goldbach receives the exact sublinear pointwise budget threshold, while Twin
Prime receives an exact block-positivity equivalence and a no-go separating
infinitude from linear density. These are partial theorems and route
...
한국어 최신 요약: 최신 탐색은 TICKET-191입니다. 콜라츠 트랙은 정확히 일곱
번의 `v=1`과 나머지 `v=2`를 갖는 모든 수축 주기족을 전 길이에서 배제했습니다.
리만 트랙은 가우스 유리수 스칼라 탐침 승격과 좌표 검사 no-go를 증명했습니다.
골드바흐는 정확한 아선형 점별 오염 예산을, 쌍둥이 소수는 블록 양성과 실제
쌍둥이 소수쌍 존재의 동치를 얻었습니다. 실제 무한 산술 전제는 남아 있고 네
문제의 해결 수는 0입니다.
...
최신 난제 연구 트랙은 TICKET-191입니다.
[탐침 위상·일곱-1 주기·정확한 산술 목표 한국어 보고서](docs/probe-sevenone-budget-granularity.ko.md)와
[영문 보고서](docs/probe-sevenone-budget-granularity.md)는 네 트랙의
정확 명제, 증명, 재현 계산, 폐기 경로와 남은 무한 간극을 분리합니다.
일곱-개 `v=1` 주기족 전체를 새로 닫았지만 실제 산술 전제는 남아 있습니다.
각 proof DAG는 TICKET-190 입력, 이번에 닫힌 정리, 폐기된 승격 경로,
다음 단일 미증명 보조정리를 분리합니다.

주요 진입점:

- [메인 연구실](https://eljja.github.io/PrimeProject/?lang=ko): TICKET-191의 현재 연구 경계, 네 문제의 부분정리·폐기 경로·남은 간극을 먼저 보고 소수 탐색기와 암호 fingerprint 실험으로 이동합니다.
- [Proof Workbench](https://eljja.github.io/PrimeProject/open-problems/index.html): 네 문제의 현재 보조정리를 한 화면에서 비교하며, 191개 티켓은 네 연구 시대와 재현 기록으로 압축해 탐색합니다.
