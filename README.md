...
The Research Atlas and Project Evolution panels read `data/project_evolution.json` and now present a condensed research narrative: the supported contribution map, evidence ladder, open-problem proof map, next academic blockers, six decisive metrics, a seventy-five-step visual change trail, a Hardening Map for recent collection-contract fixes, one Evidence Spine for artifact-backed readiness, and a claim-boundary view that separates supported controlled-synthetic results from blocked real-world and Bitcoin attribution claims. Its publication layer includes the post-pack consistency audit so the visible research history does not stop one governance step before the Evidence Pack panel.
...
The latest proof-search track is TICKET-115. It extracts the full complex constant mode from every TICKET-114 numerator block and retains the exact half-Farey cyclotomic phase sum. The complex-centered projected L2 support bound is sharp. A scalar-aware contract using `sum_q |Re(M_q H_q)|` improves the numerator budget on all six finite scales and leaves `+335,523.7` at 4M, but the gain is only `0.52%` there. An orientation-free contract using `sum_q |M_q||H_q|` is worse on all six scales and loses the 1M finite closure, so that weaker route is discarded. The next target is `EventuallySubcriticalVaughanCyclotomicMeanAndComplexCenteredNumeratorBudget`. These remain finite audits, not a Twin Prime proof. See [TICKET115](docs/twin-complex-cyclotomic-dispersion-audit.md), [TICKET114](docs/twin-ramanujan-numerator-dispersion-audit.md), and [TICKET113](docs/twin-farey-denominator-endpoint-audit.md).

한국어 최신 요약: 최신 탐색은 TICKET-115입니다. 분자 endpoint 계수의 복소 평균 전체와 half-Farey cyclotomic 위상 합을 정확히 분리했습니다. 평균의 실제 스칼라 방향 `Re(M_qH_q)`을 보존하면 여섯 규모에서 모두 조금 개선되어 4M 하한 `+335,523.7`을 남깁니다. 하지만 방향을 버리고 `|M_q||H_q|`로 처리하면 여섯 규모 모두 악화되고 1M 양성도 사라집니다. 따라서 방향 없는 복소 평균 경로는 폐기하고, 실제 Vaughan 계수에서 평균 방향과 중심화 L2 예산을 함께 제어하는 `EventuallySubcriticalVaughanCyclotomicMeanAndComplexCenteredNumeratorBudget`만 유지합니다. 네 난제는 모두 여전히 미해결입니다.
