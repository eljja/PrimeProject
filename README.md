...
The Research Atlas and Project Evolution panels read `data/project_evolution.json` and now present a condensed research narrative: the supported contribution map, evidence ladder, open-problem proof map, next academic blockers, six decisive metrics, an eighty-step visual change trail, a Hardening Map for recent collection-contract fixes, one Evidence Spine for artifact-backed readiness, and a claim-boundary view that separates supported controlled-synthetic results from blocked real-world and Bitcoin attribution claims. Its publication layer includes the post-pack consistency audit so the visible research history does not stop one governance step before the Evidence Pack panel.
...
The latest proof-search track is TICKET-121. It rationalizes the first-pair centered saving as `2w(ab-c)/(a+b+||z0+z1||)` and proves a quadratic lower certificate based on `balance * angle gap`. Exact counterexample families show that a cosine gap alone fails under norm imbalance and norm balance alone fails under near alignment. On 8 scales and 248 denominator rows, the exploratory rational thresholds `balance>=1/8`, `angle gap>=1/2` cover at least `63.8848%` of centered singleton mass; the resulting quadratic certificate is at least `17.9685%`, with zero audit failures. These thresholds were not preregistered and the finite pass is not a theorem constant. The corrected target is `VaughanLowDivisorWeightedBalanceAngleDefectGap`, followed by eventual control of every remaining canonical group and boundary term. All four conjectures remain open. See [TICKET121](docs/twin-balance-angle-defect-audit.md), [TICKET120](docs/twin-low-divisor-pair-savings-audit.md), and [TICKET119](docs/twin-canonical-pair-doubling-holdout.md).

한국어 최신 요약: 최신 탐색은 TICKET-121입니다. 첫 pair의 중심 절감을 `balance × angle gap × denominator mass`로 정확히 분해했습니다. angle gap이 최대여도 한쪽 norm이 0으로 가면 절감률이 0으로 가고, norm balance가 최대여도 두 벡터가 정렬되면 절감률이 0으로 갑니다. 따라서 TICKET-120의 angle-gap 목표를 단독 조건으로 쓰는 경로는 폐기합니다. 탐색적 유리수 임계값 `balance>=1/8`, `angle gap>=1/2`를 만족하는 중심 질량은 8개 유한 규모에서 최소 `63.8848%`였고, 이차 하한 인증률은 최소 `17.9685%`였습니다. 임계값은 사전등록되지 않았으므로 이는 유한 진단일 뿐 무한 정리가 아닙니다. 교정된 다음 목표는 `VaughanLowDivisorWeightedBalanceAngleDefectGap`이며 네 난제는 모두 미해결입니다.
