...
The Research Atlas and Project Evolution panels read `data/project_evolution.json` and now present a condensed research narrative: the supported contribution map, evidence ladder, open-problem proof map, next academic blockers, six decisive metrics, a seventy-one-step visual change trail, a Hardening Map for recent collection-contract fixes, one Evidence Spine for artifact-backed readiness, and a claim-boundary view that separates supported controlled-synthetic results from blocked real-world and Bitcoin attribution claims. Its publication layer includes the post-pack consistency audit so the visible research history does not stop one governance step before the Evidence Pack panel.
...
The latest proof-search track is TICKET-111. It decomposes the fixed-bump rational-arc correlation into exact Vaughan Type I and Type II cross-spectra. Any minor-bin partition followed by phase-blind cellwise Cauchy has an envelope at least as large as the singleton-bin envelope; that best phase-blind envelope still gives a negative lower bound at every audited horizon. At the fresh `X=2,097,152` holdout it is `7,596,484.2` against known major-plus-Type-I contribution `929,258.9`. A frozen `X^(-1/6)` candidate leaves finite lower expression `257,818.2`, but one holdout is not an infinite theorem. The next target is `PhaseAwareVaughanTypeIIMinorArcPowerSaving`. See [TICKET111](docs/twin-typeii-minor-phase-audit.md), [TICKET110](docs/twin-rational-arc-budget.md), and [TICKET109](docs/twin-spectral-phase-audit.md).

한국어 최신 요약: 최신 탐색은 TICKET-111입니다. 고정 bump 상관을 Vaughan Type I/II 교차 스펙트럼으로 정확히 분해했습니다. 위상을 버린 채 minor 주파수를 아무리 잘게 나누어 Cauchy 부등식을 적용해도 200만 holdout에서 최선 하한은 `-6,667,225.4`이므로 이 증명 계열은 닫히지 않습니다. 1M 이하에서 고정한 `X^(-1/6)` 후보는 2M에서 유한 하한 `257,818.2`를 남겼지만 무한 정리는 아닙니다. 다음 목표는 `PhaseAwareVaughanTypeIIMinorArcPowerSaving`이며 네 난제는 아직 미해결입니다.
