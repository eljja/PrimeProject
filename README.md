...
[TICKET-138: Correlation, Periodicity, and Scale Closure](docs/correlation-periodicity-and-scale-closure.md).
...
The Research Atlas and Project Evolution panels read `data/project_evolution.json` and now present a condensed research narrative: the supported contribution map, evidence ladder, open-problem proof map, next academic blockers, six decisive metrics, a ninety-nine-step visual change trail, a Hardening Map for recent collection-contract fixes, one Evidence Spine for artifact-backed readiness, and a claim-boundary view that separates supported controlled-synthetic results from blocked real-world and Bitcoin attribution claims. Its publication layer includes the post-pack consistency audit so the visible research history does not stop one governance step before the Evidence Pack panel.
...
The latest proof-search track is TICKET-139. For RH it proves that the TICKET138 absolute cross-Gram budget can overestimate a cancellation-rich tight frame by an unbounded factor. For Collatz it derives an exact Diophantine window for every positive accelerated cycle and, using the internally verified `2^28` minimum floor, excludes 19,999 of the first 20,000 period windows; the unique unexcluded period is not a cycle. For Goldbach it constructs exact signed signals on powers of two whose first arbitrarily many polynomial moments vanish, blocking moment-only pointwise inference. For Twin Prime it quantifies why scale-dependent Lipschitz regularity still permits arbitrary finite lookup and therefore cannot replace a uniform analytic norm. All four conjectures remain open. See [TICKET139](docs/uniformity-diophantine-and-complexity.md), [TICKET138](docs/correlation-periodicity-and-scale-closure.md), and [TICKET137](docs/cancellation-entropy-and-information-budget.md).

한국어 최신 요약: 최신 탐색은 TICKET-136입니다. RH에서는 core-tail 교차항의 절댓값 행합·열합으로 Schur 양성 조건을 계산할 수 있게 했고, 개별 행렬 원소가 0으로 가는 사실만으로 연산자 노름 감소를 얻는 경로를 폐기했습니다. Collatz에서는 가상의 최소 반례가 모든 비하강 접두사에서 만족해야 하는 정확한 affine 보정 부등식 `2^S n^k <= (3n+1)^k`를 증명했습니다. Goldbach에서는 고정 wheel의 rough hard stratum이 `X`에 선형이므로 powers-of-two 층의 `O(log log X)` moment 차수를 전체 층에 확대할 수 없음을 증명했습니다. Twin Prime에서는 유한 유리 Fourier 특성도 결국 유한 합동 정보이므로 같은 특성을 가진 합성수 쌍을 CRT로 만들 수 있음을 증명했습니다. 다음 핵심 보조정리는 각각 실제 Weil 행합·열합 상계, 최소 반례 valuation surplus, growing-wheel `K=56` residual 상계, 비주기 scale-growing Type II 분리입니다. 네 문제는 모두 미해결이며 완전 증명에 가깝다는 근거는 없습니다.
