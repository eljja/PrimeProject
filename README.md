...
The Baseline Lab panel reads `data/baselines/real_world/manifest.json`, `data/collection_matrix.json`, and `data/collection_power.json` to show which real-world baseline families are registered, which sample targets are still blocked, and whether those targets are only coarse screening or strong enough for tighter claims.
