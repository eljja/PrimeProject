...
The latest proof-search track is TICKET-135. It proves a sharp block-operator positivity bridge for an RH core/tail split, while leaving the actual projected Weil constants open. It proves that minimal negative-slope Collatz prefixes form a full-measure prefix-free cover in odd 2-adic code space, while explicitly rejecting the invalid promotion from almost every code to every natural orbit. It reduces the sufficient Goldbach moment order to `O(log log X)` after restriction to a sparse hard stratum, but supplies no estimate for the actual residual. It also extends the Twin Prime CRT obstruction from primorial residues to every finite exact congruence transcript. All four conjectures remain open and none is close to proof. See [TICKET135](docs/conditional-bridges-and-exceptional-set.md), [TICKET134](docs/uniformity-thresholds-and-scale-no-go.md), and [TICKET133](docs/quantifier-promotion-exact-reductions.md).

한국어 최신 요약: 최신 탐색은 TICKET-135입니다. RH에서는 core/tail 블록의 양성을 보장하는 sharp Schur-margin 정리를 증명했지만 실제 projected Weil form의 세 상수는 얻지 못했습니다. Collatz에서는 최소 기울기-수축 접두사가 홀수 2-adic 코드 공간을 거의 확실하게 덮음을 증명했지만, 자연수 코드가 측도 0이고 양의 affine 항이 남으므로 이를 모든 자연수 궤도의 하강으로 승격하는 경로를 폐기했습니다. Goldbach에서는 희소 hard stratum 안에서 필요한 moment 차수를 `O(log log X)`로 낮추는 정확한 norm bridge를 얻었지만 실제 residual 상계는 없습니다. Twin Prime에서는 임의의 유한 합동 transcript와 똑같이 보이는 합성수 쌍을 `2Lqr` 아래에 구성했습니다. 네 문제는 모두 미해결이며 완전 증명에 가깝다는 근거는 없습니다.
...
최신 난제 연구 트랙은 TICKET-135입니다. [조건부 다리·예외집합 경계 보고서](docs/conditional-bridges-and-exceptional-set.md)는 RH의 sharp block-tail 양성 조건, Collatz의 전측도 기울기-수축 cover와 자연수 null-set 간극, Goldbach 희소 hard stratum의 `O(log log X)` moment-to-maximum bridge, Twin Prime의 임의 유한 합동 transcript 합성수 lift를 한글과 영문으로 정리합니다. 다음 공격 대상은 각각 actual projected Weil Schur constants, 자연수 코드의 affine descent threshold, actual hard-stratum `K=56` moment bound, non-congruence Type II separation입니다. 네 문제는 모두 미해결입니다.
