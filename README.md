...
The latest proof-search track is TICKET-47, a periodic state-lasso stress test. It extracts a 16-edge positive-debt lasso from the 28-bit Collatz exact-template pressure graph and refutes zero-memory plus last-1 through last-4 edge-signature stateful repairs. This is a restricted no-go result for bounded suffix-memory proof routes, not a Collatz proof or counterexample; the remaining target is arbitrary finite-automaton CEGIS, 29-bit/30-bit reachability stress, or a genuinely horizon-independent ordinal measure.
...
최신 난제 연구 트랙은 TICKET-47입니다. 28-bit Collatz exact-template pressure graph에서 16-edge positive-debt lasso를 추출했고, zero-memory 및 last-1부터 last-4 edge-signature memory를 쓰는 stateful repair가 모두 같은 expanded state로 되돌아와 strict descent가 될 수 없음을 확인했습니다. 이 결론은 Collatz 증명이나 반례가 아니라 bounded suffix-memory proof route에 대한 제한적 no-go 결과입니다. 다음 목표는 arbitrary finite-automaton CEGIS, 29-bit/30-bit reachability stress, 또는 진짜 horizon-independent ordinal measure입니다.
