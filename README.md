...
The Research Atlas and Project Evolution panels read `data/project_evolution.json` and now present a condensed research narrative: the supported contribution map, evidence ladder, open-problem proof map, next academic blockers, six decisive metrics, an eighty-nine-step visual change trail, a Hardening Map for recent collection-contract fixes, one Evidence Spine for artifact-backed readiness, and a claim-boundary view that separates supported controlled-synthetic results from blocked real-world and Bitcoin attribution claims. Its publication layer includes the post-pack consistency audit so the visible research history does not stop one governance step before the Evidence Pack panel.
...
The latest proof-search track is TICKET-129. For RH it proves that finite Gaussian-rational sums of one standard smooth bump form a countable dense core inside the compact-support autocorrelation image; combined with TICKET-128, every core prime side is finite, but certified archimedean values and positivity remain open. For Collatz it combines the exact `n>=2^28` lower bound with the accelerated-orbit product identity: any hypothetical least counterexample must satisfy `S_j<=ceil(j log2 3)` for every `j<=2^29`; compatible words still survive. For Goldbach exact rational logarithm and contamination bounds improve the sufficient residual target from `K=55` to `K=56`, with margin `23019645297/2140000000000`; the pointwise residual theorem is not proved. For Twin Prime it proves the exact within-block identity `Q_X-Q_Y=(Delta A-Q_Y Delta K)/K_X` and gives a monotone endpoint countermodel, isolating the actual target `limsup D(2^j)<0.08`. Parity and exact-gap positivity remain open. All four conjectures remain open. See [TICKET129](docs/enumerable-core-valuation-cap-endpoint-budget.md), [TICKET128](docs/finite-core-prefix-constant-interpolation.md), and [TICKET126](docs/route-correction-and-premise-closure.md).

한국어 최신 요약: 최신 탐색은 TICKET-129입니다. RH에서는 하나의 표준 smooth bump를 유리 중심·scale·복소 유리 계수로 유한 결합한 집합이 compact-support 자기상관상 안에서 조밀하고 열거 가능함을 증명했습니다. TICKET-128과 결합하면 각 핵심 원소의 소수항은 유한하지만, archimedean 구간과 Weil 비음성은 아직 미증명입니다. Collatz에서는 가상의 최소 반례가 첫 `2^29` 가속 단계마다 `S_j<=ceil(j log2 3)`을 만족해야 함을 증명했지만 허용 valuation word가 남습니다. Goldbach에서는 전부 유리수인 인증으로 충분 잔차 목표를 `K=55`에서 `K=56`으로 개선했지만 점별 잔차 정리 자체는 미증명입니다. Twin Prime에서는 구간 내부 비율을 결정하는 정확한 증분 결함을 분리하고, endpoint 단조성만으로는 부족하다는 반례를 증명했습니다. 네 난제는 모두 미해결입니다.
...
최신 난제 연구 트랙은 TICKET-129입니다. [열거 핵심·valuation cap·endpoint 예산 보고서](docs/enumerable-core-valuation-cap-endpoint-budget.md)는 RH 자기상관상 내부의 열거 가능한 조밀 핵심집합, Collatz 최소 반례의 `2^29`단계 누적 valuation 상한, Goldbach `K=56` 충분조건의 유리수 인증, Twin Prime의 정확한 증분 동기화 조건을 한글과 영문으로 정리합니다. 다음 공격 대상은 각각 유리 bump 핵심 원소의 Weil 값 구간 인증, valuation-cap 언어의 전칭 소멸, 점별 Goldbach 잔차 `K<=56`, 실제 Vaughan 증분 결함의 `limsup D(2^j)<0.08` 정리입니다. 네 문제는 모두 미해결입니다.
