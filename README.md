...
The Research Atlas and Project Evolution panels read `data/project_evolution.json` and now present a condensed research narrative: the supported contribution map, evidence ladder, open-problem proof map, next academic blockers, six decisive metrics, a seventy-three-step visual change trail, a Hardening Map for recent collection-contract fixes, one Evidence Spine for artifact-backed readiness, and a claim-boundary view that separates supported controlled-synthetic results from blocked real-world and Bitcoin attribution claims. Its publication layer includes the post-pack consistency audit so the visible research history does not stop one governance step before the Evidence Pack panel.
...
The latest proof-search track is TICKET-113. It groups the 162 exact TICKET-112 endpoints by the denominator of their canonical right Farey boundary, producing 31 complex blocks without fitting signs, subsets, weights, or an exponent. On the first post-selection evaluation of this endpoint statistic at 4M, the envelope falls from `2,161,424.6` to `767,682.2`; after the unchanged variation cost, the finite lower expression is `1,017,376.2`. The 4M scale itself appeared in earlier tracks, so this is not globally unseen data. A magnitude-and-label-preserving phase adversary still gives `-376,366.3`, proving that labels alone cannot supply the missing cancellation. The next target is `UniformRightFareyDenominatorEndpointBudgetForVaughanCrossSpectrum`. These are finite floating-point audits, not a Twin Prime proof. See [TICKET113](docs/twin-farey-denominator-endpoint-audit.md), [TICKET112](docs/twin-farey-endpoint-abel-audit.md), and [TICKET111](docs/twin-typeii-minor-phase-audit.md).

한국어 최신 요약: 최신 탐색은 TICKET-113입니다. 162개 Farey cell 끝점을 바로 오른쪽 경계의 분모별 31개 복소 블록으로 정확히 묶었습니다. 새 4M holdout에서 끝점 envelope는 `2.16M`에서 `0.768M`으로 줄고, 변동항까지 포함한 유한 하한은 `+1.017M`입니다. 그러나 끝점 크기와 분모 라벨을 그대로 보존하면서 위상을 적대적으로 정렬하면 하한은 `-0.376M`이 됩니다. 즉 분모 라벨 자체가 아니라 Vaughan Type-II 계수에서 실제 위상 상쇄를 균일하게 유도해야 합니다. 다음 목표는 `UniformRightFareyDenominatorEndpointBudgetForVaughanCrossSpectrum`이며 네 난제는 여전히 미해결입니다.
