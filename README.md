...
The latest proof-search track is TICKET-49, a symbolic preimage obstruction audit. It takes the TICKET48 concrete reachability failure and identifies the first local coordinate that breaks the lasso prefix: the four 16-bit phase-compatible starts shrink to two, then one, then zero, and the unique two-step survivor reaches the third phase with `next_valuation = 5` instead of the lasso-required `1`. This is not a Collatz proof or counterexample. The remaining decisive target is an all-phase theorem proving that this next-valuation obstruction persists for every compatible lift, or a higher-bit exception that becomes a concrete periodic-lift candidate.
